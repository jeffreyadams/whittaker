...
%DIF DEL whittaker_JIMJ.tex   Fri Mar  7 09:34:01 2025
%DIF ADD revision_JIMJ.tex    Wed Feb  4 10:40:33 2026
...
\item[(ii)]  $\eta$ is a character of the nilpotent radical~$N(\R)$ of~$B(\R)$, \DIFdelbegin \DIFdel{subject to the condition that~$\eta$ }\DIFdelend \DIFaddbegin \DIFadd{which  }\DIFaddend is nontrivial on each simple root space.
\end{enumerate}
...
Recall that a representation $\pi$ of~$G(\R)$ is said to {have a $(B,\eta)$-Whittaker model} if it has a nonzero distribution vector on which $N(\R)$ acts by $\eta$ (for a more \DIFdelbegin \DIFdel{careful }\DIFdelend \DIFaddbegin \DIFadd{precise }\DIFaddend statement of the definition \DIFaddbegin \DIFadd{and terminology }\DIFaddend see Section~\ref{sec:invariants}).
...
them\DIFdelbegin \DIFdel{, defined very differently. The first is }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend the \emph{wavefront set} $\WF(\pi)$,
which is an analytic invariant\DIFdelbegin \DIFdel{.
The second is }\DIFdelend \DIFaddbegin \DIFadd{,
and }\DIFaddend the \emph{associated variety} $\AV(\pi)$, which is
...
The purpose
of this paper is to give a self-contained account of these matters. More precisely, we will\DIFdelbegin \DIFdel{do two things}\DIFdelend :
\begin{enumerate}
...
\end{enumerate}
Many results in this note \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend known. We gave new and simplified proofs for as many of them as \DIFdelbegin \DIFdel{we could}\DIFdelend \DIFaddbegin \DIFadd{possible}\DIFaddend ; for instance, Section~\ref{sec:Sekiguchi} contains a simple proof that $\WF(\pi)$ and $\AV(\pi)$ are exchanged by the Kostant--Sekiguchi correspondence. Otherwise we give convenient references.

\DIFaddbegin \DIFadd{We thank the referee for helpful suggestions.
}

\DIFaddend \section{Some invariants of irreducible representations}
...
Therefore we need to pass to the dual Lie algebra $\g^*$, and it is important to do this as independently \DIFdelbegin \DIFdel{as possible }\DIFdelend of a choice
of isomorphism $\g\simeq\g^*$ \DIFaddbegin \DIFadd{as possible}\DIFaddend . 
...
\DIFdelbegin \DIFdel{Suppose we fix a nondegenerate, $G(\R)$-invariant bilinear form $\kappa$ on $\g(\R)$. This }\DIFdelend \DIFaddbegin \DIFadd{Alternatively, this }\DIFaddend is equivalent to having a non\DIFaddbegin \DIFadd{-}\DIFaddend degenerate bilinear form on~$\g$ which is both $G(\C)$- and Galois-invariant.
The restriction of the form to each simple factor
...
$$
Using $\psi_\kappa$ we shall transfer to $\g^\ast$ several notions usually defined on $\g$: the asymptotic cone, the Kostant section, and the Kostant--Sekiguchi corres\DIFaddbegin \DIFadd{p}\DIFaddend ondence . In each case the resulting notions are independent of the
...
invariant form which is negative definite on the Lie algebra of a maximal compact subgroup\DIFdelbegin \DIFdel{.)}\DIFdelend \DIFaddbegin \DIFadd{).
}\DIFaddend 
...
$V$; we \DIFdelbegin \DIFdel{will }\DIFdelend denote it by~$\pi_K$. 
...
denoted $\AVann(\pi)$.
This is a subset of \DIFdelbegin \DIFdel{$\N^*$ of~$\g^*$}\DIFdelend \DIFaddbegin \DIFadd{$\N^* \subset \g^*$}\DIFaddend , and is the closure of a single complex nilpotent orbit: see \cite[Section 1, Theorem~4.7]{vogan_bowdoin}. Both $\AV(\pi)$ and $\AVann(\pi)$ depend only on the equivalence class of~$\pi$.
...
$$
GK(\pi)=\dim(\AV(\pi))=\frac{1}{2}\dim(\AVann(\pi)).
$$
\DIFdelbegin \DIFdel{(}\DIFdelend All dimensions are complex unless otherwise stated.
\DIFdelbegin \DIFdel{)
}\DIFdelend 
...
\subsubsection{Whittaker models}\DIFaddbegin \label{sec:whittaker_defs}\DIFaddend ~
Suppose again that $\pi$ is an irreducible admissible representation of $G(\R)$ on a Hilbert space $\mH$. We consider the corresponding actions of $G(\R)$ on the space~$\mH^\infty$ of smooth vectors in $\mH$, and on its dual $\mH^{-\infty}$. Let~$(B, \eta)$ be a pair satisfying conditions (i)-(ii) of the Introduction\DIFaddbegin \DIFadd{; we shall say $(B, \eta)$ is a }\emph{\DIFadd{Whittaker pair}}\DIFaddend .
We say $\pi$ has a \DIFdelbegin \DIFdel{$(B,\eta)$-Whittaker model
}\DIFdelend \DIFaddbegin \emph{\DIFadd{$(B,\eta)$-Whittaker model}}
\DIFaddend if there exists a nonzero element $v$ in~$\mH^{-\infty}$
such that $\pi(n)(v)=\eta(n)v$ for all $n$ in the nilradical $N(\R)$ of $B(\R)$.
See~\cite{matumoto}. 
\DIFaddbegin \DIFadd{This condition depends only on the $G(\R)$-conjugacy class of the pair $(B, \eta)$.
It is equivalent to the existence of a $G$-equivariant embedding of $\mH^\infty$
into the space of smooth sections of the equivariant line bundle over $G(\R)/N(\R)$ determined by $\eta$
(for the quasi-regular action of~$G$ on sections);
the image of that embedding
is a geometric realization of (the smooth vectors of)~$\pi$. 
This realization is the origin of the terminology ``Whittaker model for~$\pi$''. 
}

\DIFaddend If~$\w$ is the  \DIFdelbegin \DIFdel{corresponding Whittaker datum }\DIFdelend \DIFaddbegin \DIFadd{Whittaker datum corresponding to $(B, \eta)$}\DIFaddend , i.e. the $G(\R)$-conjugacy class of the pair $(B, \eta)$, we say~$\pi$ has a \DIFdelbegin \DIFdel{$\w$-Whittaker model}\DIFdelend \DIFaddbegin \emph{\DIFadd{$\w$-Whittaker model}}\DIFaddend .  
...
\subsection{Discrete series \DIFdelbegin \DIFdel{and Harish-Chandra parameters}\DIFdelend \DIFaddbegin \DIFadd{$L$-packets}\DIFaddend } \label{sec:discrete_series}

\DIFaddbegin \subsubsection{\DIFadd{Discrete series representations}}\DIFadd{~
}\DIFaddend Recall $G(\R)$ has relative discrete series representations if and only if it has a relatively compact Cartan subgroup, i.e.  \DIFdelbegin \DIFdel{there exists }\DIFdelend a $\theta$-stable Cartan subgroup~$T$ of~$G$  such that~$T(\R) \cap G_d$ is  compact, where $G_d$ is the derived group of~$G$. Throughout the paper, we assume $G(\R)$ \DIFdelbegin \DIFdel{does have relative discrete series representation}\DIFdelend \DIFaddbegin \DIFadd{has discrete series representations}\DIFaddend , we fix  a Cartan subgroup $T \subset G$ satisfying the conditions above, and we let~$\mathfrak{t}$ denote the Lie algebra of~$T$, which is a Cartan subalgebra of~$\g$. 

Choose a  positive system~$\Delta^+$  for the roots of~$\t$ in~$\g$, and let~$\rho \in \mathfrak{t}^\ast$ be the half-sum of positive roots. Let~$\Lambda \subset \mathfrak{t}^\ast$ be the set of elements~$\lambda \in \mathfrak{t}^\ast$ which are \emph{regular} (i.e. \DIFdelbegin \DIFdel{the linear form }\DIFdelend $\lambda$ does not vanish on any coroot) \DIFdelbegin \DIFdel{, }\DIFdelend and such that $\lambda-\rho$  is the differential of a character of~$T(\R)$. The \DIFaddbegin \DIFadd{set $\Lambda$ does not depend on the choice of $\Delta^+$, and is preserved by the }\DIFaddend action of the Weyl group~$W=W(\g, \t)$ \DIFdelbegin \DIFdel{preserves~$\Lambda$. 
Let~$W_K\subset W$ be the subgroup $\mathrm{Norm}_K(T)/T$. 
}\DIFdelend \DIFaddbegin \DIFadd{on $\t^\ast$. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Then any }\DIFdelend \DIFaddbegin \DIFadd{Each }\DIFaddend discrete series representation of~$G(\R)$ has \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\emph{\DIFadd{infinitesimal character}} \DIFadd{or
}\DIFaddend \emph{Harish-Chandra parameter}, which is an element of~\DIFdelbegin \DIFdel{$\Lambda/W_K$.
In fact}\DIFdelend \DIFaddbegin \DIFadd{$\Lambda/W$.
More precisely}\DIFaddend , suppose~\DIFdelbegin \DIFdel{$\lambda \in \Lambda$ and
suppose }\DIFdelend \DIFaddbegin \DIFadd{$\lambda\in\Lambda$ and
}\DIFaddend $\zeta$ is a character of the center~$Z(\R)$ of~$G(\R)$ whose
differential agrees with~$\lambda-\rho$ on the intersection $T(\R)
\cap Z(\R)$\DIFdelbegin \DIFdel{; then }\DIFdelend \DIFaddbegin \DIFadd{. Then }\DIFaddend the pair $(\lambda, \zeta)$ determines\DIFaddbegin \DIFadd{, uniquely up to unitary equivalence, }\DIFaddend a discrete
series representation~$\pi_{\lambda, \zeta}$ of~$G(\R)$\DIFaddbegin \DIFadd{: see \mbox{%DIFAUXCMD
\cite[Section~4]{Contragredient}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Every
discrete series representation arises in this way\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. The }\emph{\DIFadd{infinitesimal character}} \DIFadd{of~$\pi_{\lambda, \zeta}$ is defined to be the image of~$\lambda$ in $\mathfrak{t}^\ast/W$, and the }\emph{\DIFadd{central character}} \DIFadd{is $\zeta$. See \mbox{%DIFAUXCMD
\cite[Section~8]{AV1} }\hskip0pt%DIFAUXCMD
}\DIFaddend and \DIFdelbegin \DIFdel{$\pi_{\lambda, \zeta}=\pi_{\lambda', \zeta'}$ if and only if $\zeta=\zeta'$ and $\lambda, \lambda'$ are $W_K$-conjugate. See \mbox{%DIFAUXCMD
\cite[Section 8]{AV1} }\hskip0pt%DIFAUXCMD
and }\DIFdelend \cite{Contragredient} for more on the parametrization.

\DIFdelbegin \DIFdel{Recall that the }\DIFdelend \DIFaddbegin \DIFadd{There is a simple criterion for genericity in terms of this parametrization. Let us say a set of positive roots $\Delta^+$, or the corresponding Weyl
chamber, is }\DIFaddend \emph{\DIFdelbegin \DIFdel{Weyl chambers}%DIFDELCMD < \MBLOCKRIGHTBRACE %%%
\DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{large}} \DIFadd{if every simple root is non-compact.
For $\lambda\in \Lambda$, let $\Delta^+(\lambda)$ be the set of roots $\alpha$ for which the coroot~$\ch\alpha$ satisfies $\langle \ch\alpha,\lambda\rangle>0$.
Suppose $\pi\simeq\pi_{\lambda,\zeta}$ is a discrete series representation; then $\pi$ is large if and only
if $\Delta^+(\lambda)$ is large.
See~\mbox{%DIFAUXCMD
\cite[Section~6]{vogan-gelfand-kirillov}}\hskip0pt%DIFAUXCMD
. In the terminology of Shelstad~\mbox{%DIFAUXCMD
\cite[Section~2.3]{shelstad_inner_forms}}\hskip0pt%DIFAUXCMD
, if $B$~is the corresponding Borel subgroup of~$G$ corresponding to $\Delta^+(\lambda)$, then $(B, T)$ is a fundamental Borel pair of Whittaker type.
}


\subsubsection{\DIFadd{Discrete series $L$-packets}}\DIFadd{~
By definition an }\emph{\DIFadd{$L$-packet of discrete series representations}} \DIFadd{for $G(\R)$ is precisely the set of discrete series with a given
infinitesimal character and a given central character. Each $L$-packet is a finite set, and each discrete series representation belongs to a single $L$-packet. 
}

\DIFadd{The group $W$ has an action on  the set of equivalence classes of discrete series representations, given by $w\cdot \pi_{\lambda,\zeta}=\pi_{w\lambda,\zeta}$. This action preserves,
and is transitive on, each $L$-packet}\DIFaddend ~\DIFdelbegin \DIFdel{$\mathfrak{t}^\ast$ are the connected components of the set of regular elements. Each Weyl chamber specifies a positive root system for the roots of $\mathfrak{t}$ in~$\g$, }\DIFdelend \DIFaddbegin \DIFadd{$\Pi$.
Furthermore the stabilizer of any  $\pi\in \Pi$ is the normal subgroup $W_K=\mathrm{Norm}_K(T)/T$ of~$W$. For all this see \mbox{%DIFAUXCMD
\cite[Section~4]{Contragredient}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{To discuss the generic elements in a discrete series $L$-packet let us introduce the adjoint group $\Gad=G/Z(G)$, and set $\Kad=(\Gad)^\theta$.
The Weyl groups of $G$ }\DIFaddend and \DIFdelbegin \DIFdel{we say a Weyl chamber is }\emph{\DIFdel{large}} %DIFAUXCMD
\DIFdel{if the corresponding simple roots are all noncompact}\DIFdelend \DIFaddbegin \DIFadd{$\Gad$ are canonically isomorphic, so we can identify $W_{\Kad}$ with a subgroup of $W=W(G,T)$. It is immediate from the definition that for $g\in \Norm_G(T)$,  the image of $g$ in $W$
is in $W_{\Kad}$ if and only if $\int(g)$ (conjugation by $g$) commutes with $\theta$, or equivalently $g\inv \theta(g)\in Z(G)$}\DIFaddend .


\DIFdelbegin \DIFdel{If~$\lambda \in \mathfrak{t}^\ast$ }\DIFdelend \DIFaddbegin \begin{lemma}
  \label{l:bijectiongeneric}
  \DIFadd{Suppose $\pi=\pi_{\lambda,\zeta}$ }\DIFaddend is a \DIFdelbegin \DIFdel{representative for the Harish-Chandra parameter of a }\DIFdelend \DIFaddbegin \DIFadd{generic }\DIFaddend discrete series
  representation\DIFaddbegin \DIFadd{. Then $\pi_{w\lambda,\zeta}$ is generic if and only
  if $w\in W_{\Kad}$. The map $w \mapsto \pi_{w\lambda}$ induces a bijection between $Q=W_{\Kad}/W_K$ and the generic elements of the L-packet~$\Pi$ of }\DIFaddend $\pi$,
  \DIFdelbegin \DIFdel{then~$\pi$ is
}\DIFdelend \DIFaddbegin \DIFadd{and the action of~$W$ on~$\Pi$ induces a simply transitive action of $Q$ on~$\Pi$.
}\end{lemma}

\begin{proof}
 \DIFadd{Suppose $g\in \Norm_G(T)$, with image $w\in W$.
By the discussion preceding the Lemma,
we have to show every simple root of $\Delta^+(w\lambda)$ is
non-compact if and only if the endomorphism $\int(g)$ commutes with~$\theta$.  Suppose
$\alpha$ is a simple root for $\Delta^+(\lambda)$, so by assumption it
is non-compact.  If the endomorphism $\int(g)$ commutes with $\theta$, then 
$\theta(\int(g)X_\alpha)=\int(g)(\theta(X_\alpha))=-\int(g)(X_\alpha)$,
so $\int(g)\alpha=w\alpha$ is also non-compact.
Conversely if $\int(g)X_\alpha$ is non-compact then $\theta(\int(g)X_\alpha)=\int(g)\theta(X_\alpha)$,
and writing any root as a sum of simple roots we see $\theta$ commutes with $\int(g)$.
}

\DIFadd{The second assertion follows from the fact that $W$ acts transitively on the $L$-packet: by the first part an element $w\in W$ preserves the property of being }\DIFaddend generic if and only if \DIFdelbegin \DIFdel{the Weyl chamber containing~$\lambda$ is large.
See ~\mbox{%DIFAUXCMD
\cite[Section 6]{vogan-gelfand-kirillov}}\hskip0pt%DIFAUXCMD
. 
 }\DIFdelend \DIFaddbegin \DIFadd{$w\in W_{\Kad}$, and the stabilizer of any element is $W_K$.
}\end{proof}
\DIFaddend 

\DIFaddbegin \begin{remark}
\DIFadd{For alternative descriptions of $Q=W_{\Kad}/W_K$ see Section \ref{sec:first_defs_q}.
There we shall see that it is an elementary abelian $2$-group;
that it is trivial when $G$ is adjoint;
and that when $G$ is simple,  the cardinality of~$Q$ is at most~$4$.
See Remark~\ref{rk:size_Q} and Lemma~\ref{lem:third_Q}. 
 }\end{remark}



\DIFaddend \subsection{The invariants for generic discrete series}
\medskip
...
Let~$\mathfrak{t}^+$ be a Weyl chamber in~$\mathfrak{t}^\ast$ containing a representative~$\lambda$ for the Harish-Chandra parameter of~$\pi$. 
\DIFdelbegin \DIFdel{Write }\DIFdelend \DIFaddbegin \DIFadd{Define  }\DIFaddend $\lim_{x \to 0(\mathfrak{t}^+)}\left[ G(\R)\cdot x\right]$ \DIFdelbegin \DIFdel{for the set of all elements $y \in i\g(\R)^\ast$ such that every $G(\R)$-invariant neighborhood of~$y$ contains all elements of~$\mathfrak{t}^+$ sufficiently close to zero}\DIFdelend \DIFaddbegin \DIFadd{as in \mbox{%DIFAUXCMD
\cite{rossmann_limit_orbits}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
By Lemma B in~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{rossmann_limit_orbits}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin {\it \DIFadd{loc.cit.}}\DIFaddend , if $\mathfrak{t}^+$ is a large Weyl chamber, then $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ is a single $G(\R)$-orbit.  Since this \DIFdelbegin \DIFdel{does happen }\DIFdelend \DIFaddbegin \DIFadd{is the case }\DIFaddend when~$\pi$ is a generic discrete series representation, to prove assertion~(c)  it is enough to check the closure of $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ contains $\WF(\pi)$ in that case. Now, the equivalence between conditions  (L1) and (L2) on page 2 of~\cite{rossmann_limit_orbits} implies that the closure of $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ contains the asymptotic cone of the orbit $G(\R)\cdot \lambda$  (see  Section~\ref{sec:asymp_cone} below). Therefore assertion~(c) follows from the fact that this asymptotic cone is equal to  $\WF(\pi)$  (Lemma~\ref{lem:WF_and_AC} below).
\end{proof}
...
prove, using elementary arguments, that \DIFdelbegin \DIFdel{(3) $\leftrightarrow$ (4)
}\DIFdelend \DIFaddbegin \DIFadd{the bijection \mbox{(3) $\leftrightarrow$ (4)} we give 
}\DIFaddend coincides with the Kostant--Sekiguchi correspondence.



Let us now explain the strategy of the proof, which is mainly an exercise in putting together some references that are rather scattered in the literature. What we shall actually do is: \DIFdelbegin \DIFdel{introduce a }\DIFdelend \DIFaddbegin \DIFadd{consider the }\DIFaddend finite group~\DIFdelbegin \DIFdel{$Q(G)$}\DIFdelend \DIFaddbegin \DIFadd{$Q(G)=W_{\Kad}/W_K$ from Section~\ref{sec:discrete_series}}\DIFaddend , and point out that 
\begin{itemize}
\item[(i)] \DIFdelbegin \DIFdel{it acts }\DIFdelend \DIFaddbegin \DIFadd{$Q(G)$ acts simply }\DIFaddend transitively on~(1)\DIFdelbegin \DIFdel{, and simply transitively on (2)}\DIFdelend --(4);
\item[(ii)] the maps $\pi \mapsto \Wh(\pi)$, $\pi \mapsto \AV(\pi)$ and $\pi\mapsto \WF(\pi)$  are equivariant for these actions.
\end{itemize}
\DIFdelbegin \DIFdel{The theorem }\DIFdelend \DIFaddbegin 


\DIFadd{We discussed the simply transitive action on~(1) in Section~\ref{sec:discrete_series}; for the definitions of the actions on (2)-(4) and the other properties in~(i)-(ii)  see Section \ref{sec:actions}. This will use several realizations for the group $Q(G)$, spelled out in Section~\ref{s:Q}.
}


\DIFadd{The theorem then }\DIFaddend follows immediately from \DIFdelbegin \DIFdel{these two observations , plus the  following basic fact on group actions.
}%DIFDELCMD < 

%DIFDELCMD < \begin{lemma} %%%
\DIFdel{Suppose a group}\DIFdelend \DIFaddbegin \DIFadd{observations (i)-(ii) and from the  elementary fact that if a group~}\DIFaddend $Q$ acts \DIFaddbegin \DIFadd{simply transitively }\DIFaddend on two sets $A$ and~$B$\DIFdelbegin \DIFdel{. Let $f\colon A \to B$ be a }\DIFdelend \DIFaddbegin \DIFadd{, then any }\DIFaddend $Q$-equivariant map \DIFdelbegin \DIFdel{. 
}%DIFDELCMD < \begin{itemize}
\begin{itemize}%DIFAUXCMD
%DIFDELCMD < \item[(a)] %%%
\item[\DIFdel{(a)}]%DIFAUXCMD
\DIFdel{If $Q$ acts transitively on~$B$, then ~$f$ is surjective}\textup{\DIFdel{;}}
%DIFAUXCMD
%DIFDELCMD < \item[(b)] %%%
\item[\DIFdel{(b)}]%DIFAUXCMD
\DIFdel{If $Q$ acts transitively on~$A$ and freely on~$B$, then ~$f$ is injective.
}
\end{itemize}%DIFAUXCMD
%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdel{In particular, if~$Q$ acts transitively on ~$A$ and simply transitively on~$B$, then~$f$ must be a bijection and the  action of ~$Q$ on~}\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend $A$ \DIFdelbegin \DIFdel{must be simply transitive. }%DIFDELCMD < \end{lemma}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{to $B$ must be bijective. 
}\DIFaddend 



\DIFaddbegin \begin{example}
\DIFadd{If $G(\R)$ is the symplectic group $\mathrm{Sp}(2n, \R)$, then $Q(G) \simeq \Z/2\Z$ and there are two distinct Whittaker data for $G(\R)$. If $\Pi$ is the $L$-packet of discrete series with the same infinitesimal character as the trivial representation, then $\Pi_{\mathrm{gen}}$ has two elements, and the correspondence $\pi \mapsto \Wh(\pi)$ in Theorem~\ref{th:main} was made explicit by Mœglin and Renard \mbox{%DIFAUXCMD
\cite[Proposition~4.1]{moeglin_renard}}\hskip0pt%DIFAUXCMD
, based on work of Wallach~\mbox{%DIFAUXCMD
\cite{wallach} }\hskip0pt%DIFAUXCMD
and Oda~\mbox{%DIFAUXCMD
\cite{oda} }\hskip0pt%DIFAUXCMD
for $n=1,2$. As we shall see (in Section~\ref{sec:alternate_WF}), this result agrees with our description of  $\pi \mapsto \Wh(\pi)$, given in Section~\ref{s:gds}. }\end{example} 


\begin{remark} 
\DIFadd{Not all quasi-split real groups have discrete series representations. But if $G(\R)$ is a quasi-split real group, then the  }\emph{\DIFadd{fundamental series}} \DIFadd{representations that are generic are irreducibly induced from generic discrete series of a maximal cuspidal Levi subgroup $M(\R)$ of $G(\R)$. Using \mbox{%DIFAUXCMD
\cite[Chapter~14]{abv}}\hskip0pt%DIFAUXCMD
, especially Lemma 14.11 there, one may relate Whittaker data for $G(\R)$ with Whittaker data for $M(\R)$, and the generic fundamental series representations in a given $L$-packet for $G(\R)$ (together with the attached Whittaker data) with  generic discrete series representations for maximal cuspidal Levi subgroups. We shall not pursue this here. 
 }\end{remark} 




\DIFaddend \subsection{The groups $Q_\sigma(G)$ and $Q_{\theta}(G)$}\DIFaddbegin \label{s:Q}
\DIFaddend 

\subsubsection{Definition of the two groups, and \DIFaddbegin \DIFadd{a }\DIFaddend canonical isomorphism}\DIFaddbegin \label{sec:first_defs_q}\DIFaddend ~
...
\DIFaddbegin \begin{remark}\label{rk:size_Q} \DIFadd{When $G$ is adjoint, it immediately follows from the definitions that $(G_\ad)^\sigma = (G^\sigma)_{\ad}$, therefore $Q_\sigma(G)$ is trivial in that case. 
}

\DIFadd{In general, the group $G(\R)_{\ad}=(G^\sigma)_{\ad}$ contains the identity component of~$\Gad(\R)$, because the above injection $(G^\sigma)_{\ad} \hookrightarrow (\Gad)^\sigma$ is an isomorphism on the Lie algebra level. Therefore  $Q_\sigma(G)$ may be viewed as a quotient of the component group of $\Gad(\R)$; since the latter is an elementary abelian $2$-group, the same holds for $Q_\sigma(G)$. 
}

\DIFadd{When $G(\R)$ is connected, which happens for instance if $G$ is simply connected, the group $G(\R)_{\ad}$ is itself connected, and therefore equal to the identity component of~$\Gad(\R)$. In that case $Q_\sigma(\R)$ is the component group of~$\Gad(\R)$.
}

\DIFadd{For the concrete computation of this component group in case  $G$ is simple, see the tables in \mbox{%DIFAUXCMD
\cite[Section~10]{galois}}\hskip0pt%DIFAUXCMD
. In particular  $Q_\sigma(\R)$ has at most four elements in that case.
}\end{remark}

\DIFaddend A similar discussion applies to the involution $\theta$.
Then $Z^\theta$ is the center of $K=G^\theta$ and  $\theta$ factors to $\Gad$.
We define $(G^\theta)_\ad=G^\theta/Z^\theta$ and
$$
Q_\theta(G)=(\Gad)^\theta/(G^\theta)_\ad\DIFdelbegin \DIFdel{=(\Gad)^\theta/\Kad}\DIFdelend .
$$
...
\DIFdelbegin \DIFdel{The proof uses }\DIFdelend \DIFaddbegin \DIFadd{We shall prove this in Section~\ref{sec:proof_lemma}, using }\DIFaddend an interpretation of $Q_\sigma(G)$ and $Q_\theta(G)$ in terms of group cohomology \DIFdelbegin \DIFdel{, which we  now spell out }\DIFdelend \DIFaddbegin \DIFadd{which we  spell out in the next section.
Before that, let us point out that the groups $Q_\sigma(G)$, $Q_\theta(G)$ are isomorphic with the group $W_{\Kad}/W_K$ of Section~\ref{sec:discrete_series}.
}

\DIFadd{By definition every element $w$ in $W_{\Kad}$ has a representative $g$ in $(\Gad)^\theta$; the image of~$g$ in $Q_\theta(G)=(\Gad)^\theta/(G^\theta)_\ad$ depends only on~$w$. This induces a group homomorphism 
 $\psi\colon W_{\Kad} \to Q_\theta(G)$}\DIFaddend .

 \DIFaddbegin \begin{lemma}\label{lem:third_Q}
  \DIFadd{The morphism  $\psi$ induces an isomorphism $W_{\Kad}/W_K \to Q_\theta(G)$. 
}\end{lemma}
\begin{proof} 
\DIFadd{Let us identify $W_{\Kad}$ with a subgroup of $W(G, T)$ as in Section~\ref{sec:discrete_series}. Then for $w \in W_{\Kad}$, the element $\psi(w)$ is trivial if and only  if $w$ has a representative $g \in \mathrm{Norm}_G(T)$ which is $\theta$-invariant, and this means $w \in W_K$. Therefore the kernel of $\psi$ is $W_K$. 
}

\DIFadd{Given $q \in Q_\theta(G)$ and a representative $g \in (\Gad)^{\theta}$, the $\theta$-invariant maximal tori $g^{-1} T_{\ad} g$ and $T_{\ad}$ of~$\Gad$ are conjugate by an element $x \in (G^\theta)_\ad$; then the element $xg$ of $(\Gad)^\theta$ normalizes $T_{\mathrm{ad}}$, and its image $w$ in $W_{\Kad}$ satisfies $\psi(w)=q$. Thus $\psi$ is surjective.
}\end{proof}

\DIFadd{Because of Lemmas~\ref{lem:iso_q} and~\ref{lem:third_Q}, when describing the actions in Theorem~\ref{th:main},  we shall realize $Q(G)$  as $W_{\Kad}/W_K$, as $Q_\sigma(G)$ or as~$Q_\theta(G)$, depending on the situation.
}



\DIFaddend \subsubsection{Connection with group cohomology}~
...
\end{equation}
\DIFdelbegin \DIFdel{In~\eqref{long_ptset} the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend connecting map  $ \psi_\tau\colon \Gad^{\tau} \rightarrow H^1(\tau,Z)$ is defined as follows: if $\gamma \in \Gad^{\tau}$ is the coset\DIFaddbegin \DIFadd{~}\DIFaddend $gZ$, then $g\tau(g^{-1}) \in Z^{-\tau}$, and $\psi_\tau$ sends~$\gamma$ to the class of $g \tau(g^{-1})$ in $H^1(\tau, Z)$. The kernel of~$\psi_\tau$ is $(G^\tau)_{\ad}$. Since~\eqref{long_ptset} is exact we deduce that $\psi_\tau$ induces a canonical  bijection
...
\subsubsection{Proof of Lemma~\ref{lem:iso_q}}~\DIFaddbegin \label{sec:proof_lemma}
\DIFaddend 
...
\DIFaddbegin \label{sec:actions}
\DIFaddend 


\subsubsection{On representations}\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \label{sec:action_DS}\DIFadd{~
}



\DIFaddend The adjoint action of $\Gad^\sigma$ on~$G(\R)$ induces an action on $G(\R)$-representations, which descends to an action of  $Q_\sigma(G)$ on equivalence class of $G(\R)$-representations. If~$q$ is an element of~$Q_\sigma(G)$, we write $\pi \mapsto \pi^q$ for the action of~$q$ on equivalence classes of $G(\R)$-representations. 

Similarly the actions of $\Gad^\theta$ on~$\g$ and~$K$ induce an action of $Q_\theta(G)$ on equivalence classes of $(\g,K)$-modules, for which we use \DIFdelbegin \DIFdel{analogous }\DIFdelend \DIFaddbegin \DIFadd{similar }\DIFaddend notation.
...
\DIFdelbegin \DIFdel{Because of Lemmas}\DIFdelend \DIFaddbegin \DIFadd{There is a similar compatibility statement with the third description of $Q(G)$ as $W_{\Kad}/W_K$, and the corresponding action on equivalence classes of discrete series representations. Suppose $q \in Q_{\sigma}(G)$, and let $w$ be the element of $W_{\Kad}/W_K$ that corresponds to $q$ using the isomorphisms of Lemma}\DIFaddend ~\ref{lem:iso_q} and\DIFdelbegin \DIFdel{\ref{lem:action_q}, we write $Q(G)$ for the group }\DIFdelend \DIFaddbegin \DIFadd{~\ref{lem:third_Q}. Suppose $\pi=\pi_{\lambda, \zeta}$ is the equivalence class of a discrete series representation of~$G(\R)$; inspecting of the definitions in \mbox{%DIFAUXCMD
\cite[Section~4]{Contragredient} }\hskip0pt%DIFAUXCMD
shows that $w\cdot \pi = \pi^q$. 
}

\DIFadd{As a result, if~$\Pi$ is an $L$-packet of discrete series for $G(\R)$, we have compatible actions on~$\Pi$ of $Q(G)=W_{\Kad}/W_K$, of }\DIFaddend $Q_\sigma(G)$\DIFdelbegin \DIFdel{or~$Q_\theta(G)$, depending on the situation. }\DIFdelend \DIFaddbegin \DIFadd{, and of $Q_{\theta}(G)$. These actions are all simply transitive by Lemma~\ref{l:bijectiongeneric}.
}\DIFaddend 


\DIFaddbegin \subsubsection{\DIFadd{On Whittaker data and generic representations}}\label{sec:action_whitt}\DIFadd{~ 
Suppose $\tilde{q} \in \Gad^\sigma$. For every Whittaker pair $(B, \eta)$ as in Section~\ref{sec:whittaker_defs}, the pair $\tilde{q}\cdot (B, \eta)=\big(\tilde{q} B \tilde{q}^{-1}, \eta \circ (x \mapsto \tilde{q} x \tilde{q}^{-1})\big)$ is again a Whittaker pair, which is conjugate to $(B, \eta)$ if and only if $\tilde{q} \in (G^\sigma)_{\ad}$. This determines an action of $Q_\sigma(G)$ on the set of Whittaker data for~$G(\R)$, which we shall denote by $(q, \mathfrak{w}) \mapsto q \cdot \mathfrak{w}$. This action is simply transitive: see \mbox{%DIFAUXCMD
\cite[Lemma 3.1.3]{adams_kaletha} }\hskip0pt%DIFAUXCMD
for an elementary argument.
}

\DIFadd{Now let $\pi$ be a generic discrete series representation of~$G(\R)$. Let $q$ be an element of $Q_\sigma(G)$, with a representative $\tilde{q}$ in $\Gad^\sigma$.  For every Whittaker pair $(B, \eta)$ such that $\pi$ has a $(B, \eta)$-Whittaker model, it is immediate from the definitions that the representation $\pi^{\tilde g}$ is again a discrete series representation, and that it has a $\tilde{q}\cdot (B, \eta)$-Whittaker model. Passing to equivalence classes we see that $\pi^q$ is generic, and satisfies $\Wh(\pi^q)=q \cdot \Wh(\pi)$. Therefore the map $\pi \mapsto \Wh(\pi)$ is $Q_\sigma(G)$-equivariant.
  }



\DIFaddend \subsubsection{On real nilpotent orbits} \DIFaddbegin \label{sec:action_WF}\DIFaddend ~
To define the action of~$Q_{\sigma}(G)$ on~(4), first observe that~$\Gad$ acts on~$\g$ by the adjoint action. This action preserves~$\Op$, and its restriction to~$\Gad(\R)$ preserves~$\Op\cap i\g(\R)^*$. Since elements \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend a given~$G(\R)_{\mathrm{ad}}$-orbit are in the same~$G(\R)$-orbit, this induces an action of $Q_{\sigma}(G)=\Gad(\R)/G(\R)_{\mathrm{ad}}$ on~$(\Op\cap i\g(\R)^*)/G(\R)$.
...
\DIFdelbegin \DIFdel{On the other hand }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{On regular nilpotent $K$-orbits}}\label{sec:action_AV}\DIFadd{~ 
Next, }\DIFaddend $Q(G)$, realized as $Q_\theta(G)$, is a group of automorphisms of $(\g,K)$.
...
In this section only we need to  choose a particular  \DIFdelbegin \DIFdel{an }\DIFdelend isomorphism \mbox{$\gder\simeq \gder^*$}.
...
Let $\overline{\mathfrak b}(\R)$ be the kernel of $X$ \DIFaddbegin \DIFadd{in $\g(\R)$}\DIFaddend . This is the Lie algebra of~$\overline{B}(\R)$, where
...
character of the nilradical\DIFaddbegin \DIFadd{~}\DIFaddend $N(\R)$  of $B(\R)$\DIFaddbegin \DIFadd{, }\DIFaddend by
\[ 
...
Using Lemma~\ref{lem:Kost_and_AC} again, we see that all \DIFdelbegin \DIFdel{those }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend orbits $\mathcal{O}'$ satisfy $F \in \AC(\mathcal{O}')$,
...
\subsubsection{The Sekiguchi correspondence for principal nilpotent orbits}\label{sec:concrete_sek}\DIFaddbegin \DIFadd{~
}\DIFaddend Let us spell out the special case of the Sekiguchi correspondence which is of interest here. We follow~\cite[Section~2]{AVAV}.


Suppose $E_\R\in i\g(\R)$ is a principal nilpotent element.
We may choose \DIFdelbegin \DIFdel{$H_\R, E_\R \in \g$ }\DIFdelend \DIFaddbegin \DIFadd{$H_\R, F_\R \in \g$ }\DIFaddend such that $(H_\R,E_\R,F_\R)$ is an $\mathrm{SL}(2)$-triple and  $E_\R$ \DIFaddbegin \DIFadd{is }\DIFaddend contained in $i\g(\R)$,
which implies $H_\R\in \g(\R)$. 
After conjugating by~$G(\R)$ we may further assume $\theta(E_\R)=-F_\R$. Set 
\[   E_\theta=\frac{1}{2}(E_\R-F_\R+H_\R)\quad
F_\theta=\frac{1}{2}(-E_\R+F_\R+H_\R).
\]
Then
$$
...
E_\R=\frac{1}{2}(E_\theta-F_\theta-H_\theta)\quad
F_\R=\frac{1}{2}(-E_\theta+F_\theta-H_\theta).
\end{equation}
Then
\begin{equation}
\label{e:KS}
...
\subsubsection{A simple proof of Proposition~\ref{Sekiguchi_result}}\label{sec:sekiguchi_proof}\DIFaddbegin \DIFadd{~
}\DIFaddend 

Let us fix a generic discrete \DIFaddbegin \DIFadd{series }\DIFaddend representation~$\pi$ and let $\lambda$ be \DIFaddbegin \DIFadd{a representative for }\DIFaddend its Harish-Chandra parameter.
As in Section \ref{s:AV} \DIFdelbegin \DIFdel{we }\DIFdelend let $\kappa$ be the Killing form and $\psi\colon\gder\to \gder^*$ be the corresponding isomorphism.
...
\[ \begin{aligned}
  F_\R&=\frac{1}{2}(-E_\theta+F_\theta-H_\theta)\\
  E_\R&=\frac{1}{2}(E_\theta-F_\theta-H_\theta)\\
...
$$
\WF(\pi)=G(\R)\cdot \psi(F_\R), \quad \text{where $F_\R = \frac{i}{2}(-E_\theta+F_\theta-H_\theta)\in\N\cap i\g(\R)$.}
$$\end{lemma}
\DIFaddbegin 

\begin{example}
\DIFadd{Suppose $G(\R) = \mathrm{Sp}(2n, \R)$ and ~$\pi$ is the generic discrete series representation denoted  $\pi_{\mathbf{B}_+, \mathbf{T}}$ in \mbox{%DIFAUXCMD
\cite[Section~4.2]{moeglin_renard}}\hskip0pt%DIFAUXCMD
. As in Section~\ref{s:AV},  fix a nonzero root vector~$X_\alpha$  for each simple root $\alpha$ of $\mathbf{T}$ in the Borel subgroup $\mathbf{B}_+$. These determine a pinning $\mathcal{P}=(\mathbf{T}, \mathbf{B}_+, \{X_\alpha\})$ of~$G$. Following \mbox{%DIFAUXCMD
\cite[Section 3.1, see before Lemma 3.1.2]{adams_kaletha}}\hskip0pt%DIFAUXCMD
, we may attach to $\mathcal{P}$ a principal nilpotent element $X_\mathcal{P}$; with this notation it follows from \mbox{%DIFAUXCMD
\cite[Lemma 3.1.2]{adams_kaletha} }\hskip0pt%DIFAUXCMD
that the Whittaker data $\mathfrak{w}_{\psi(F_\R)}$  and  $\mathfrak{w}_{X_\mathcal{P}}$ are identical. By Lemma~\ref{lem:explicit_WF} and Corollary~\ref{cor:matumoto}, in the terminology of \mbox{%DIFAUXCMD
\cite{adams_kaletha}}\hskip0pt%DIFAUXCMD
, this means $\Wh(\pi)$ is the Whittaker datum attached to the pinning~$\mathcal{P}$ and the additive character  $\Lambda\colon x \mapsto e^{ix}$ of~$\R$. The latter is the Whittaker datum denoted~$\Wh_1$ in \mbox{%DIFAUXCMD
\cite[Section~4.2]{moeglin_renard}}\hskip0pt%DIFAUXCMD
, and therefore our results are consistent with \mbox{%DIFAUXCMD
\cite[Proposition~4.1]{moeglin_renard}}\hskip0pt%DIFAUXCMD
. 
}\end{example} 


\DIFaddend There is a partial converse to \DIFdelbegin \DIFdel{this statement }\DIFdelend \DIFaddbegin \DIFadd{Lemma~\ref{lem:explicit_WF} }\DIFaddend if we consider discrete series within a single $L$-packet. Fix an $L$-packet~$\Pi$ of discrete series, and consider a regular semisimple orbit~$\Omega$ in~$\Op \cap i\g(\R)^\ast$; then we can determine the generic representation~$\pi \in \Pi$ with $\WF(\pi)=\Omega$, as follows. Fix~$\xi_\R \in \Omega$, write $\xi_\R=F_\R^\ast$ with $F_\R \in \g$ and let $E_\R, H_\R \in \g$ be such that $(E_\R, H_\R, F_\R)$ is an  $\mathrm{SL}(2)$-triple with $\theta(E_\R)=-F_\R$. Set
$$
H_\theta=E_\R-F_\R.
$$
...
$$
E_\theta=\frac{1}{2}\begin{pmatrix}1&i\\i&-1\end{pmatrix}, \quad
F_\theta=\frac{1}{2}\begin{pmatrix}1&-i\\-i&-1\end{pmatrix},\quad
...
$$
  E_\theta=\frac{1}{2}(E_\R-F_\R+H_\R), \quad 
  F_\theta=\frac{1}{2}(-E_\R+F_\R+H_\R), \quad
  H_\theta=-E_\R-F_\R
$$
and
$$
   E_\R=\frac{1}{2}(E_\theta-F_\theta-H_\theta), \quad 
  F_\R=\frac{1}{2}(-E_\theta+F_\theta-H_\theta), \quad 
  H_\R=E_\theta+F_\theta.
$$
...
$\alpha=\lambda_{2i}$; therefore $\rho=\frac{1}{2}\alpha$ is~$\lambda_i$.
...
&=  \psi_c(\KS(K\cdot \psi_c\inv (E_\theta^*)))\\
  &=  \psi_c(\KS(K\cdot \frac{1}{c} F_\theta))\\
&=\psi_c(G(\R)\cdot \frac{1}{c}(-E_\R))\quad\text{(by \eqref{e:KS})}\\
...
$$
\begin{aligned}
\K(-E_\R^*)&=\psi_c(\K(\psi_c\inv(-E_\R^*))) =\psi_c(\K( -\frac{1}{c}F_\R)).\end{aligned}$$
This contains
$$ \psi_c\left(\frac{1}{c}(-F_\R-E_\R)\right)=\psi_c(\frac{1}{c}H_\theta)=2H_\theta^*.
$$
So we do have
$$
