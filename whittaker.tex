...
We recall what it means for a representation $\pi$ to have a $(B,\eta)$-Whittaker model in Section ?.
...
Suppose $(\pi,V)$ is an irreducible representation of $G(\R)$  on a Hilbert space. 
The underlying
...
Now suppose $(\pi,V)$ is an irreducible representation of $G(\R)$ on a Hilbert space. 
...
In the other direction if $\pi$ is an irreducible representation of $G(\R)$ we define $\AV(\pi)$ to be the associated variety 
of its underlying $(\g,K)$-module. 

[Need a paragraph explaining $\Wh(\pi)$, following Matsumoto]
...
Note that $Q_\sigma(G)$ is a set of automorphisms of $G(\R)$. It therefore acts on representations of $G(\R)$. 
Similarly $Q_\theta(G)$ is a set of automorphisms of $(\g,K)$, and acts on $(\g,K)$-modules. 
...
Based on these two Lemmas we write $Q(G)$ for the group $Q_\sigma(G)$ or $Q_\theta(G)$, depending on the situation.
