...
We need to pass to the dual Lie algebra $\g^*$.
We say an element of $\g^*$ is nilpotent if its $G$-orbit is a (weak) cone, i.e. closed under multiplication by $\C^*$,
and \emph{regular} if $\dim_{\Cent(G)}(X)=\mathrm{rank}(G)$.  
The principal nilpotent orbit $\O_p\in \g^*$ is the set of regular nilpotent elements.


The statements of the main results do not require the choice of an isomorphism $\g\simeq \g^*$.
In several places we do need to make such a choice;
with the exception of (?) this choice will not matter.

So we fix a non-degenerate, $G(\R)$-invariant bilinear form $\kappa$ on $\g(\R)$. This is equivalent to having a non-degenerate, $G(\C)$ and Galois invariant
bilinear form on $\g$. The restriction of the form each simple factor
is unique up to a non-zero constant. For example we can use the Killing form on $\gder$, extended arbitrarily by a non-degenerate form on the center.

Given $\kappa$ we define
$\psi_\kappa: \g\rightarrow \g^*$ as usual:
$$
\psi_\kappa(X)(Y)=\kappa(X,Y)\quad (X,Y\in\g).
$$

Using $\psi_\kappa$ we transfer the asymptotic cone, Kostant section, and Kostant Sekiguchi corresondence to $\g^*$. In each case these constructions are independent of the
choice of $\kappa$. See (?).
...
denoted $\AVann(\pi)$.
This is a subset of $\N^*\subset\g^*$, and is the closure of a single complex nilpotent orbit: see \cite[Section 1, Theorem 4.7]{vogan_bowdoin}.
...
For this we let $\kappa$ be the killing form, and define $\psi:\gder\rightarrow \gder^*$ by
$$
\psi(X)(Y)=\kappa(X,Y)\quad (X,Y\in \gder).
$$
...
Then $F_\pi\in\s$ is a regular nilpotent element.
Then 
$$
\AV(\pi)=K\cdot \psi(F_\pi).
$$
\end{lemma}
See
...
For this see \cite{HarrisHeOlafsson}; in the special casea considered here, the result is due to Rossmann and can be extracted from \cite{RossmannPicard}, Theorems B and C. \qed
...
The following discussion is taken directly from \cite[Section~3.1]{adams_kaletha}.


Suppose $X\in i\g(\R)^*\subset \g^*$ is a regular nilpotent element (see Section \ref{s:dual}).
Let $\overline{\mathfrak b}(\R)$ be the kernel of $X$. 
The corresponding Borel subgroup $B$ is defined over $\R$.
...
$$
Since $\psi$ is unique on each simple factor up to non-zero scalar, it is easy to see this definition is independent of the choice of $\psi$. See \cite[Section 3.2]{adams_kaletha}.
...
Let $\mathcal{O}$ be a regular semisimple orbit and  let $X$ be a regular nilpotent element. Then $\Kostant{X}$ meets~$\mathcal{O}$ if and only if $X$ is in the asymptotic cone~$\AC(\mathcal{O})$. 
\end{lemma}

For a closely related result see \cite[Proposition 3.5]{fm}.

The Proposition, Lemma and Lemma \ref{lem:WF_and_AC} give the following result,
...
Write $\O_H$ for the $G(\R)$-orbit of $H$.
...
The second assertion is an immediate consequence of the first and Lemma \ref{lem:Kost_and_AC}
...
\subsection{$\AV(\pi)$ and $\WF(\pi)$: the Kostant--Sekiguchi correspondence } \label{sec:Sekiguchi}
...
It is well known that a natural bijection between nilpotent $G(\R)$-orbits on $\g(\R)$ and nilpotent $K$ orbits on $\s$. 
This is the Kostant--Sekiguchi
correspondence~\cite{sekiguchi}, which we write as $\KS(\O)$. It is a deep theorem of
Schmid and Vilonen (the main result of~\cite{SV1}) that the wavefront
set and the associated variety are exchanged by this
...
\KS(\AV(\pi))=\WF(\pi)
$$
In other words the bijection (3)--(4) is induced by the Kostant--Sekiguchi correspondence.
\end{proposition}
...
We need the corresponding result in $\g^*$. As discussed in Section \ref{s:dual} this uses a choice of an isomorphism $\g\simeq \g^*$, but the result is independent of this choice.
That is for $X\in \O_p\subset \g^*$ we have
$$
\KS(K\cdot X)=\psi_c(\KS(K\cdot \psi_c\inv(X)),
$$
...
As in Section \ref{s:AV} we let $\kappa$ be the Killing form and write $\psi:\gder\simeq \gder^*$ for the corresponding isomorphism.
...
We need to show this equals $\WF(\pi)$. By Corollary \ref{cor:matumoto} it is enough to show $\Wh(\pi)=\w_{\psi(-E-\R)}$; by 
  by Lemma \ref{lem:Kost_and_AC} this is equivalent to $\K(\psi(-E_\R))$ meets $G(\R)\cdot\lambda$. 
...
Let $\t(\C)=\{t_z\mid z\in\C\}$ where
$$
t_z=\begin{pmatrix}0&z\\-z&0
\end{pmatrix}.
$$
Then $\t(\R)=\{t_x\mid x\in\R\}$ is the Lie algebra of a compact Cartan subalgebra of $G$ (this is also a maximal compact subgroup).
...
Let's compute $\WF(\pi_{\lambda_{ik}})$ by  \cite{HarrisHeOlafsson}, which says:
$$
\WF(\pi(\lambda_{ik}))=\AC(G(\R)\cdot \lambda_{ik}).
$$

First let's compute $\AC(G(\R)\cdot kH_\theta)=\AC(G(\R)\cdot\begin{pmatrix}0&-ik\\ik&0
\end{pmatrix})$ for $k\in \Z_{\ne 0}$.
Here we're taking the real asymptotic cone; since $kH_\theta\in i\g(\R)$ this is a subset of $i\g(\R)$. 
...
Now we carry out this computation over to $i\g(\R)^*$:
$$
\begin{aligned}
...
\end{aligned}
$$
Note that $c$ canceled as expected.
In the last line we've used that $E_\R^*,F_\R^*$ are $G(\R)$-conjugate.
...
\subsection{$\AV(\pi(\lambda_{ik}))$}

We apply Lemma \ref{l:AV}. It is easy to see
$$
F_{\pi(\lambda_{ik})}=
\begin{cases} F_\theta&k>0\\
  E_\theta&k<0
\end{cases}
$$
Recall (Section \ref{s:AV}) $\psi$ is defined using the Killing form $\kappa_4$, which satisfies
...
Let's check Proposition \ref{Sekiguchi_result} in this case.
...
&=G(\R)\cdot (-E_\R^*)\\
&=\WF(\pi)\quad \text{(by \ref{e:WF})}
\end{aligned}
$$
which agrees with 
Proposition \ref{Sekiguchi_result}.

Next, let's check Proposition \ref{p:Kostant}, 
...
&=\psi_c(\K( -\frac 1cF_\R))\\
&\ni \psi_c\frac 1c(-F_\R-E_\R)\\
&=\psi_c(\frac 1cH_\theta)=2H_\theta^*
\end{aligned}
$$
That is 
$$
