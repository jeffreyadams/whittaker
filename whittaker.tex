...
Recall $G(\R)$ has relative discrete series representations if and only if it has a relatively compact Cartan subgroup. From now on, we assume $G(\R)$ does have discrete series representation, and fix a compact Cartan subgroup $H(\R)$ of~$G(\R)$, with complexified Lie algebra~$\mathfrak{h} \subset \g$. 

With this choice, any discrete series representation of~$G(\R)$ has a \emph{Harish-Chandra parameter}. Etc etc.
\begin{itemize}
\item Harish-Chandra parameter, $W$ and $W_K$
...
Let~$\mathfrak{h}^+$ be a Weyl chamber in~$\mathfrak{h}$ containing a representative for the Harish-Chandra parameter of~$\pi$. Write $\lim_{x \to 0(\mathfrak{h}^+)} G(\R)\cdot x$ for the set of all elements $y \in \g$ such that every $G$-invariant neighborhood of~$y$ contains all elements of~$\mathfrak{h}^+$ sufficiently close to zero. By Lemma B in~\cite{rossmann_limit_orbits}, if $\mathfrak{h}^+$ is a large Weyl chamber, then $\lim_{x \to 0(\mathfrak{h}^+)} G\cdot x$ is a single $G$-orbit.  Since this does happen when~$\pi$ is a generic discrete series representation, to prove assertion~(c)  it is enough to check the closure of $\lim_{x \to 0(\mathfrak{h}^+)} G\cdot x$ contains $\WF(\pi)$ in that case. Now, the equivalence between conditions  (L1) and (L2) on page 2 of~\cite{rossmann_limit_orbits} immediately imply that the closure of $\lim_{x \to 0(\mathfrak{h}^+)} G\cdot x$ contains the asymptotic cone of the orbit of the Harish-Chandra parameter (see  Section~\ref{sec:asymp_cone} below). Therefore assertion~(c) follows from the fact that this asymptotic cone is equal to  $\WF(\pi)$  (Lemma~\ref{lem:WF_and_AC} below).
