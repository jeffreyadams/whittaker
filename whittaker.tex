...
We need to pass to the dual Lie algebra $\g^*$, and it is important to do this as canonically as possible, i.e. independent of a choice
of isomorphism $\g\simeq\g^*$. First we define the nilpotent cone in $\g^*$ as follows. 
...
The statements of the main results do not require the choice of an isomorphism $\g\simeq \g^*$. However such a choice appears in the proofs so we spell this out.
Having a $G(\R)$-invariant bilinear form on $\g(\R)$ is equivalent to having a $G(\R)$-equivariant isomorphism $\g(\R)\simeq \g^*(\R)$. 
Suppose we fix a non-degenerate, $G(\R)$-invariant bilinear form $\kappa$ on $\g(\R)$. This is equivalent to having a non-degenerate, $G(\C)$ and Galois invariant
...
We illustrate these results with the illustrative example of $SL(2,\R)$,.
...
Let's check Proposition \ref{Sekiguchi_result} in this case, which says $\KS(\AV(\pi))=\WF(\pi)$.
...
$$
The case $k<0$ is similar; we conclude
