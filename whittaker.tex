...
We work mostly in the  algebraic setting of
$(\g,K)$-modules. However, mainly for the analytic wave front set
invariant we need to consider representations of $G(\R)$ on a Banach
space. Suppose $(\pi,V)$ is an irreducible representation of $G(\R)$ 
 where $V$ is a Banach space. The underlying
$(\g,K)$-module is on the space of $K$-finite vectors in
$V$. Conversely if $\pi$ is an irreducible $(\g,K)$-module it can be
realized as the $K$-finite vectors of some  Banach space
representation.
...
Now suppose $(\pi,V)$ is an irreducible representation of $G(\R)$. 
See \cite{howe_wave_front}, \cite{bv_local_structure},
\cite{HarrisHeOlafsson} for the definition of the \emph{wave-front
  set} $\WF(\pi)$. This is an analytic invariant determined by the
singularities of the global character of $\pi$ near the identity. 
...
\begin{lemma}
There is a canonical isomorphism $\phi:Q_\sigma(G)\simeq Q_\theta(G)$ defined as follows.
Suppose $x\in Q_\sigma(G)=(\Gad)^\sigma/(G^\sigma)_\ad$.
We can choose a representative $g\in (\Gad)^\sigma$ of $x$ which is also in $(\Gad)^\theta$.
Define $\phi(x)$ to be the image of $g$ in $Q_\theta(G)=(\Gad)^\theta/(G^\theta)_\ad$.
This is independent of the choice of $g$ and $\phi$ is an isomorphism of pointed sets.
\end{lemma}

Based on the Lemma we define $Q(G)$ to be $Q_\sigma(G)$ or $Q_\theta(G)$, depending on the setting.

[This is an explicit version of the group cohomology argument. Some detail here...]
...
\subsection{Action of $Q(G)$ on representations and invariants}
