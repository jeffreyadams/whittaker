...
\end{enumerate}
Many results in this note were known. We gave new and simplified proofs for as many of them as we could; for instance, Section~\ref{sec:Sekiguchi} contains a simple proof that $\WF(\pi)$ and $\AV(\pi)$ are exchanged by the Kostant--Sekiguchi correspondence. Otherwise we give convenient references.
...
\subsubsection{Involutions}~
 By definition there exists an anti-holomorphic involution~$\sigma$ of~$G$ such that $G(\R) = G^\sigma$. Fix such an involution~$\sigma$, and choose a Cartan involution $\theta$ for $G$. This means:
