...
if there exists a nonzero element $v$ in~$\mH^{-\infty}$ such that $\pi(n)(v)=\eta(n)v$ for all $n$ in the nilradical $N(\R)$ of $B(\R)$. See~\cite{matumoto}. \textcolor{red}{[NEW]} This condition depends only on the $G(\R)$-conjugacy class of the pair $(B, \eta)$. It is equivalent to the existence of a $G$-equivariant embedding of $\mH^\infty$ into the space of smooth sections of the equivariant line bundle over $G(\R)/N(\R)$ determined by $\eta$ (for the quasi-regular action of~$G$ on sections); the image of that embedding is a geometric realization of (the smooth vectors of)~$\pi$ which is the origin of the terminology ``Whittaker model for~$\pi$''. \textcolor{red}{[/NEW]} 
 ...
 If~$\w$ is the corresponding Whittaker datum, i.e. the $G(\R)$-conjugacy class of the pair $(B, \eta)$, we say~$\pi$ has a $\w$-Whittaker model.  
...
\textcolor{red}{[NEW]} For the action on~(1) see Section~\ref{sec:action_DS}. For the action on~(2) and the equivariance of $\pi \mapsto \Wh(\pi)$, see Section~\ref{sec:action_whitt}. For the action on~(4) and the equivariance of the map $\pi \mapsto \AV(\pi)$, see Section~\ref{sec:action_AV}. For the action on~(3) and the equivariance of $\pi \mapsto \WF(\pi)$ see Section~\ref{sec:action_WF}. \textcolor{red}{[/NEW]}
...
\begin{center} \textcolor{red}{TO DO : explain that this preserves generic representations, and  DS representations within a single $L$-packet.\\ Then explain why the action is transitive.   }\end{center} 

\subsubsection{\textcolor{red}{(NEW)} On Whittaker data}\label{sec:action_whitt}

\begin{center} \textcolor{red}{TO DO : explain how $Q_\sigma(G)$ acts on Whittaker data, and refer to \cite[Lemma 3.1.3]{adams_kaletha} for the simple transitivity; point out that $\pi \mapsto \Wh(\pi)$ is equivariant.   }\end{center} 
