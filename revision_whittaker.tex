...
For the action on~(1) see Section~\ref{sec:action_DS}. For the action on~(2) and the equivariance of $\pi \mapsto \Wh(\pi)$, see Section~\ref{sec:action_whitt}. For the action on~(4) and the equivariance of the map $\pi \mapsto \AV(\pi)$, see Section~\ref{sec:action_AV}. For the action on~(3) and the equivariance of $\pi \mapsto \WF(\pi)$ see Section~\ref{sec:action_WF}. 
...
\begin{remarkplain} \textup{The group $Q$ is a finite abelian group, in fact an elementary abelian $2$-group. It is trivial when $G$ is adjoint, and when $G$ is a simple group the cardinal of~$Q$ is at most~$4$. See Remark~\ref{rk:size_Q} for a more precise discussion.} \end{remarkplain} 
