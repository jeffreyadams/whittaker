...
\subsubsection{Parametrization and infinitesimal character}
...
If $\pi=\pi_{\lambda,\zeta}$ is a discrete series representation then $\pi$ is generic if and only
...
By definition $L$-packet of discrete series representations for $G(\R)$ is precisely the set of discrete series with a given
infinitesimal and a given central character. This is a finite set. 
...
\begin{remarkplain}{For alternative descriptions of $W_{\Kad}/W_K$ see Section \ref{s:Q}.
There we shall see that it is an elementary abelian $2$-group; it is trivial when $G$ is adjoint; and when $G$ is simple  the cardinality of~$Q$ is at most~$4$. See Remark~\ref{rk:size_Q} for the proof and for more precise information.} \end{remarkplain}
...
\begin{itemize}
\item[(i)] $Q(G)$ acts transitively on~(1), and simply transitively on (2)--(4);
\item[(ii)] the maps $\pi \mapsto \Wh(\pi)$, $\pi \mapsto \AV(\pi)$ and $\pi\mapsto \WF(\pi)$  are equivariant for these actions.
\end{itemize}


We discussed the transitive action on~(1) in Section~\ref{sec:discrete_series}; for the  actions on (2)-(4) and the properties asserted in~(i)-(ii)  see Section \ref{sec:actions}.
\begin{comment}
...
\end{comment}
The theorem then follows immediately from these observations, and from the following basic fact on group actions, whose proof is left to the reader.
...
We work with $Q_\theta(G)$.  If $g\in (\Gad)^\theta$, choose $x\in (G^\theta)_{\mathrm{ad}}$ so that $xg\in \Norm_G(T)$.
This induces a well-defined map
 $\psi\colon W_{\Kad} \to Q_\theta(G)$,
and it is easy to see the kernel is precisely $W_K$. Therefore:
 \begin{lemma}
  The map  $\psi\colon W_{\Kad} \to Q_\theta(G)$ induces an isomorphism between $Q(G)=W_{\Kad}/W_K$ and $Q_\theta(G)$. 
  Consequently $Q(G)$ acts simple transitively on the generic members of an L-packet of discrete series.
\end{lemma}
...
Because of Lemmas~\ref{lem:iso_q} and \ref{lem:action_q}, we write $Q(G)$ for the group $Q_\sigma(G)$ or~$Q_\theta(G)$, depending on the situation.\footnote{AA 2026-I-14:  we should spell out that if $\pi$ is a discrete series then this fits with the action of $W_{\Kad}/W_K$ defined in Sect.~\ref{sec:discrete_series}.}
