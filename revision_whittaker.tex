...
\cap Z(\R)$. Then the pair $(\lambda, \zeta)$ determines a discrete
series representation~$\pi_{\lambda, \zeta}$ of~$G(\R)$. Every
discrete series representation arises in this way.

We say a set of positive roots $\Delta^+$, or the corresponding Weyl
chamber, is large if every simple root is non-compact.
If $\pi=\pi_{\lambda,\zeta}$ is a discrete series representation the $\pi$ is generic if and only
...
An L-packet of discrete series representations is precisely the set of discrete series with given
infinitesimal and central characters.
There is an action of $W$ on the set of discrete series representations: $w\cdot \pi_{\lambda,\zeta}=\pi_{w\lambda,\zeta}$.
This action preserves,
and is transitive on, each L-packet $\Pi$.
Furthermore the stabilizer of any  $\pi\in \Pi$ is $W_K$. 

Let $\Gad=G/Z(G)$ be the adjoint group of $G$, and $\Kad=(\Gad)^\theta$.
...
is in $W(\Kad)$ if and only if $\int(g)$ (conjugation by $g$)  commutes with $\theta$, or equivalently
$g\inv \theta(g)\in Z(G)$.
...
\begin{lemma}
  ...
  if $w\in W_{\Kad}$, and the generic elements of the L-packet of $\pi$ are in bijection with $W_{\Kad}/W_K$.
\end{lemma}
...
The second assertion follows from the fact that $W$ acts transitively on the L-packet; $w\in W$ preserves the property of being generic if and only if $w\in W_{\Kad}$, and the stabilizer of any element is $W_K$.
\end{proof}

We will see shortly that $W_{\Kad}/W_K$ has an alternative description as the group $Q(G)$ (Section \ref{s:Q}).
...
\begin{itemize}
\item[(i)] $Q$ acts transitively on~(1), and simply transitively on (2)--(4);
\item[(ii)] the maps $\pi \mapsto \Wh(\pi)$, $\pi \mapsto \AV(\pi)$ and $\pi\mapsto \WF(\pi)$  are equivariant for these actions.
\end{itemize}


For details on these actions and their properties see Section \ref{sec:actions}.
\begin{comment}
