...
the: the \emph{wavefront set} $\WF(\pi)$,
...
is in $W_{\Kad}$ if and only if $\int(g)$ commutes with $\theta$, or equivalently
$g\inv \theta(g)\in Z(G)$.
...
\begin{lemma}
  Suppose $\pi=\pi_{\lambda,\zeta}$ is generic discrete series
...
notation.  Suppose $g\in \Norm_G(T)$. By the discussion preceding the
...
$\theta(\int(g)X_\alpha)=\int(g)(\theta(X_\alpha))=-\int(g)\theta(X_\alpha)$,
...
We will see shortly alternative descriptions for the group $W_{\Kad}/W_K$: see Section \ref{s:Q}.
...
\begin{remarkplain}{The group $Q$ is an elementary abelian $2$-group. It is trivial when $G$ is adjoint, and when $G$ is simple  the cardinality of~$Q$ is at most~$4$. See Remark~\ref{rk:size_Q} for the proof and for more precise information.} \end{remarkplain}
