...
is in $W(\Kad)$ if and only if $\int(g)$ commutes with $\theta$, or equivalently
$g\inv \theta(g)\in Z(G)$.
...
\begin{lemma}
  ...
  if $w\in W_{\Kad}$, and the generic elements of the L-packet of $\pi$ are in bijection with $W(\Kad)/W(K)$.
\end{lemma}
...
The second assertion follows from the fact that $W$ acts transitively on the L-packet; $w\in W$ preserves the property of being generic if and only if $w\in W(\Kad)$, and the stabilizer of any element is $W_K$.
\end{proof}
...
\begin{lemma}
  The group $Q(G)$ is isomorphic to $W_{\Kad}/W_K$ of Section \ref{sec:discrete_series}.
  Consequently $Q(G)$ acts simple transitively on the generic members of an L-packet of discrete series.
\end{lemma}

\begin{proof}
This is straightforward. We work with $Q_\theta$.  If $g\in (\Gad)^\theta$, choose $x\in (G^\theta)_{\mathrm{ad}}$ so that $xg\in \Norm_G(T)$. This induces a well-defined map
