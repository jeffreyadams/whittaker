...
Recall that a representation $\pi$ of~$G(\R)$ is said to {have a $(B,\eta)$-Whittaker model} if it has a nonzero distribution vector on which $N(\R)$ acts by $\eta$ (for a more careful statement of the definition and terminology see Section~\ref{sec:invariants}).
...
\textcolor{red}{[NEW]} For the action on~(1) see Sections~\ref{sec:action_DS} and~\ref{sec:action_transitivity}. For the action on~(2) and the equivariance of $\pi \mapsto \Wh(\pi)$, see Section~\ref{sec:action_whitt}. For the action on~(4) and the equivariance of the map $\pi \mapsto \AV(\pi)$, see Section~\ref{sec:action_AV}. For the action on~(3) and the equivariance of $\pi \mapsto \WF(\pi)$ see Section~\ref{sec:action_WF}. \textcolor{red}{[/NEW]}
...
\begin{remarkplain} \textcolor{red}{[NEW]} {The group $Q$ is an elementary abelian $2$-group. It is trivial when $G$ is adjoint, and when $G$ is simple  the cardinal of~$Q$ is at most~$4$. See Remark~\ref{rk:size_Q} for a more precise discussion.} \end{remarkplain}
...
\begin{center} \textcolor{red}{TO DO : explain that the action of $Q_\sigma$ on generic DS preserves $L$-packets.\\ Then explain why the action on is transitive.\\
Perhaps adding something about $L$-packets in \S~\ref{sec:discrete_series} would be useful. }\end{center} 
