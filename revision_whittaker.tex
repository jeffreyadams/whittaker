...
if there exists a nonzero element $v$ in~$\mH^{-\infty}$ such that $\pi(n)(v)=\eta(n)v$ for all $n$ in the nilradical $N(\R)$ of $B(\R)$. See~\cite{matumoto}. \textcolor{red}{[NEW]} This condition depends only on the $G(\R)$-conjugacy class of the pair $(B, \eta)$. It is equivalent to the existence of a $G$-equivariant embedding of $\mH^\infty$ into the space of smooth sections of the equivariant line bundle over $G(\R)/N(\R)$ determined by $\eta$ (for the quasi-regular action of~$G$ on sections); the image of that embedding is a geometric realization of (the smooth vectors of)~$\pi$ which is the origin of the terminology ``Whittaker model for~$\pi$''. \textcolor{red}{[/NEW]} 
...
Recall that the \emph{Weyl chambers} of~$\mathfrak{t}^\ast$ are the connected components of the set of regular elements. Each Weyl chamber specifies a positive root system for the roots of $\mathfrak{t}$ in~$\g$, \textcolor{red}{[new]} and a Borel subgroup of~$G$ containing~$T$ and defined over~$\R$ \textcolor{red}{[/new]}. We say a Weyl chamber is \emph{large}, or that the corresponding Borel subgroup is large, if the corresponding simple roots are all noncompact. 

If~$\lambda \in \mathfrak{t}^\ast$ is a representative for the Harish-Chandra parameter of a discrete series representation $\pi$, then~$\pi$ is generic if and only if the Weyl chamber containing~$\lambda$ is large. See~\cite[Section 6]{vogan-gelfand-kirillov}. \textcolor{red}{[new]} In the terminology of Shelstad~\cite[Section 2.3]{shelstad}, if $B$~is the corresponding Borel subgroup of~$G$, then $(B, T)$ is a fundamental Borel pair of Whittaker type.\textcolor{red}{[/new]}
...
\begin{remarkplain} \textcolor{red}{[NEW]} \label{rk:size_Q} When $G$ is adjoint, it immediately follows from the definitions that $(G_\ad)^\sigma = (G^\sigma)_{\ad}$, therefore $Q_\sigma(G)$ is trivial in that case. 
...
For the concrete computation of this component group in case  $G$ is simple, see the tables in \cite[Section~10]{galois}. In particular  $Q_\sigma(\R)$ has at most four elements in that case.\textcolor{red}{[/NEW]}
\end{remarkplain}
...
\subsubsection{\textcolor{red}{(NEW SEC.)} On Whittaker data and generic representations}\label{sec:action_whitt}
