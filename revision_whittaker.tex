...
\item $\eta$ is a character of the nilpotent radical~$N(\R)$ of~$B(\R)$, which  is nontrivial on each simple root space.
\end{enumerate}
The existence of a Borel subgroup satisfying (i) is the definition of $G$ being quasisplit, which we assume holds from now on.

Recall that a representation $\pi$ of~$G(\R)$ is said to {have a $(B,\eta)$-Whittaker model} if it has a nonzero distribution vector on which $N(\R)$ acts by $\eta$ (for a precise statement of the definition and terminology see Section~\ref{sec:invariants}).
...
them: the \emph{wavefront set} $\WF(\pi)$,
which is an analytic invariant, an the \emph{associated variety} $\AV(\pi)$, which is
...
of this paper is to give a self-contained account of these matters. More precisely, we will:
\begin{enumerate}
...
\end{enumerate}
Most of the  results in this note are well known. We give new and simplified proofs for as many of them as possible; for instance, Section~\ref{sec:Sekiguchi} contains a simple proof that $\WF(\pi)$ and $\AV(\pi)$ are exchanged by the Kostant--Sekiguchi correspondence. Otherwise we give convenient references.
...
Let~$\sigma$ be the anti-holomorphic involution defining~$G(\R)$: $G(\R)=G^\sigma$.
...
Therefore we need to pass to the dual Lie algebra $\g^*$, and it is important to do this as independently of a choice
of isomorphism $\g\simeq\g^*$ as possible.
...
Alternatively, this is equivalent to having a non-degenerate bilinear form on~$\g$ which is both $G(\C)$- and Galois-invariant. The restriction of the form to each simple factor
...
Using $\psi_\kappa$ we shall transfer to $\g^\ast$ several notions usually defined on $\g$: the asymptotic cone, the Kostant section, and the Kostant--Sekiguchi correspondence. In each case the resulting notions are independent of the
...
We  pass back and forth between representations of $G(\R)$ and $(\g,K)$-modules.
...
\subsubsection*{Whittaker models}\label{sec:whittaker_defs}

Suppose again that $\pi$ is an irreducible admissible representation of $G(\R)$ on a Hilbert space $\mH$. We consider the corresponding actions of $G(\R)$ on the space~$\mH^\infty$ of smooth vectors in $\mH$, and on its dual $\mH^{-\infty}$. Let~$(B, \eta)$ be a pair satisfying conditions (i)-(ii) of the Introduction; we shall say $(B, \eta)$ is a \emph{Whittaker pair}. We say $\pi$ has a \emph{$(B,\eta)$-Whittaker model}
if there exists a nonzero element $v$ in~$\mH^{-\infty}$ such that $\pi(n)(v)=\eta(n)v$ for all $n$ in the nilradical $N(\R)$ of $B(\R)$. See~\cite{matumoto}.This condition depends only on the $G(\R)$-conjugacy class of the pair $(B, \eta)$. It is equivalent to the existence of a $G$-equivariant embedding of $\mH^\infty$ into the space of smooth sections of the equivariant line bundle over $G(\R)/N(\R)$ determined by $\eta$ (for the quasi-regular action of~$G$ on sections); the image of that embedding is a geometric realization of (the smooth vectors of)~$\pi$ which is the origin of the terminology ``Whittaker model for~$\pi$''. 

 If~$\w$ is the corresponding Whittaker datum, i.e. the $G(\R)$-conjugacy class of the pair $(B, \eta)$, we say~$\pi$ has a \emph{$\w$-Whittaker model}.  
...
\subsection{Discrete series $L$-packets} \label{sec:discrete_series}

\subsubsection{Discrete series representations}
Recall $G(\R)$ has relative discrete series representations if and only if it has a relatively compact Cartan subgroup, i.e.  a $\theta$ and $\sigma$-stable Cartan subgroup~$T$ of~$G$  such that~$T(\R) \cap G_d$ is  compact, where $G_d$ is the derived group of~$G$. Throughout the paper, we assume $G(\R)$  has discrete series representations, we fix  a Cartan subgroup $T \subset G$ satisfying the conditions above, and we let~$\mathfrak{t}$ denote the Lie algebra of~$T$, which is a Cartan subalgebra of~$\g$. 

Choose a  positive system~$\Delta^+$  for the roots of~$\t$ in~$\g$, and let~$\rho \in \mathfrak{t}^\ast$ be the half-sum of positive roots. Let~$\Lambda \subset \mathfrak{t}^\ast$ be the set of elements~$\lambda \in \mathfrak{t}^\ast$ which are \emph{regular} (i.e. $\lambda$ does not vanish on any coroot), and such that $\lambda-\rho$  is the differential of a character of~$T(\R)$. The action of the Weyl group~$W=W(\g, \t)$ preserves~$\Lambda$. Let~$W_K\subset W$ be the subgroup $\mathrm{Norm}_K(T)/T$. If $\lambda\in \Lambda$ let $\Delta^+(\lambda)$ be the roots $\alpha$ so that $\langle \ch\alpha,\lambda\rangle>0$.


Each discrete series representation of~$G(\R)$ has a
\emph{Harish-Chandra parameter}, which is an element of~$\Lambda$.
More precisely, suppose~$\lambda\in\Lambda$ and
$\zeta$ is a character of the center~$Z(\R)$ of~$G(\R)$ whose
differential agrees with~$\lambda-\rho$ on the intersection $T(\R)
\cap Z(\R)$. Then the pair $(\lambda, \zeta)$ determines, uniquely up to unitary equivalence, a discrete
series representation~$\pi_{\lambda, \zeta}$ of~$G(\R)$. Every
discrete series representation arises in this way. The \emph{infinitesimal character} of~$\pi_{\lambda, \zeta}$ is defined to be the image of~$\lambda$ in $\mathfrak{t}^\ast/W$. See \cite[Section~8]{AV1} and \cite{Contragredient} for more on the parametrization.

We say a set of positive roots $\Delta^+$, or the corresponding Weyl
chamber, is large if every simple root is non-compact.
If $\pi\simeq\pi_{\lambda,\zeta}$ is a discrete series representation, then $\pi$ is large if and only
if $\Delta^+(\lambda)$ is large.
See~\cite[Section~6]{vogan-gelfand-kirillov}. In the terminology of Shelstad~\cite[Section 2.3]{shelstad}, if $B$~is the corresponding Borel subgroup of~$G$, then $(B, T)$ is a fundamental Borel pair of Whittaker type.



\subsubsection{Discrete series $L$-packets}
By definition an \emph{$L$-packet of discrete series representations} for $G(\R)$ is precisely the set of discrete series with a given
...
The group $W$ has an action on  the set of equivalence classes of discrete series representations, given by $w\cdot \pi_{\lambda,\zeta}=\pi_{w\lambda,\zeta}$. This action preserves,
and is transitive on, each $L$-packet $\Pi$.
Furthermore the stabilizer of any  $\pi\in \Pi$ is $W_K$. For all this see \cite[Section~4]{Contragredient}.

To discuss the generic elements in a discrete series $L$-packet let us introduce the adjoint group $\Gad=G/Z(G)$, and set $\Kad=(\Gad)^\theta$.
The Weyl groups of $G$ and $\Gad$ are canonically isomorphic, so we can identify $W_{\Kad}$ with a subgroup of $W=W(G,T)$. It is immediate from the definition that if $g\in \Norm_G(T)$ then the image of $g$ in $W$
is in $W_{\Kad}$ if and only if $\int(g)$ (conjugation by $g$) commutes with $\theta$, or equivalently $g\inv \theta(g)\in Z(G)$.


\begin{lemma}
  \label{l:bijectiongeneric}
  Suppose $\pi=\pi_{\lambda,\zeta}$ is a generic discrete series
  representation. Then $\pi_{w\lambda,\zeta}$ is generic if and only
  if $w\in W_{\Kad}$. The map $w \mapsto \pi_{w\lambda}$ induces a bijection between $W_{\Kad}/W_K$ and the generic elements of the L-packet of $\pi$.
\end{lemma}

\begin{proof}
The central character $\zeta$ plays no role so we drop it from the
notation.  Suppose $g\in \Norm_G(T)$ with image $w\in W$.
By the discussion preceding the
Lemma we have to show every simple root of $\Delta^+(w\lambda)$ is
non-compact if and only if $\int(g)$ commutes with $\theta$.  Suppose
$\alpha$ is a simple root for $\Delta^+(\lambda)$, so by assumption it
is non-compact.  If $\int(g)$ commutes with $\theta$, then
...
The second assertion follows from the fact that $W$ acts transitively on the $L$-packet: by the first part an element $w\in W$ preserves the property of being generic if and only if $w\in W_{\Kad}$, and the stabilizer of any element is $W_K$.
\end{proof}

\begin{remarkplain}
For alternative descriptions of $W_{\Kad}/W_K$ see Section \ref{sec:first_defs_q}.
...
Define  $\lim_{x \to 0(\mathfrak{t}^+)}\left[ G(\R)\cdot x\right]$ as in \cite{rossmann_limit_orbits}. 
By Lemma B in~{\it loc.cit.} if $\mathfrak{t}^+$ is a large Weyl chamber, then $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ is a single $G(\R)$-orbit.  Since this is the case when~$\pi$ is a generic discrete series representation, to prove assertion~(c)  it is enough to check the closure of $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ contains $\WF(\pi)$ in that case. Now, the equivalence between conditions  (L1) and (L2) on page 2 of~\cite{rossmann_limit_orbits} implies that the closure of $\lim_{x \to 0(\mathfrak{t}^+)} \left[G(\R)\cdot x\right]$ contains the asymptotic cone of the orbit $G(\R)\cdot \lambda$  (see  Section~\ref{sec:asymp_cone} below). Therefore assertion~(c) follows from the fact that this asymptotic cone is equal to  $\WF(\pi)$  (Lemma~\ref{lem:WF_and_AC} below).
\end{proof}
...
prove, using elementary arguments, that the bijection (3) $\leftrightarrow$ (4) we give
coincides with the Kostant--Sekiguchi correspondence.



Let us now explain the strategy of the proof, which is mainly an exercise in putting together some references that are rather scattered in the literature. What we shall actually do is: consider the finite group~$Q(G)=W_{\Kad}/W_K$ from Section~\ref{sec:discrete_series}, and point out that 
\begin{itemize}
\item[(i)] $Q(G)$ acts simply transitively on~(1)--(4);
\item[(ii)] the maps $\pi \mapsto \Wh(\pi)$, $\pi \mapsto \AV(\pi)$ and $\pi\mapsto \WF(\pi)$  are equivariant for these actions.
\end{itemize}


We discussed the simply transitive action on~(1) in Section~\ref{sec:discrete_series}; for the definitions of the actions on (2)-(4) and the other properties in~(i)-(ii)  see Section \ref{sec:actions}. This will use several realizations for the group $Q(G)$, spelled out in Section~\ref{s:Q}.
\begin{comment}
  For the action on~(1) see Sections~\ref{sec:action_DS} and~\ref{sec:action_transitivity}. For the action on~(2) and the equivariance of $\pi \mapsto \Wh(\pi)$, see Section~\ref{sec:action_whitt}. For the action on~(4) and the equivariance of the map $\pi \mapsto \AV(\pi)$, see Section~\ref{sec:action_AV}. For the action on~(3) and the equivariance of $\pi \mapsto \WF(\pi)$ see Section~\ref{sec:action_WF}.
\end{comment}

The theorem then follows immediately from observations (i)-(ii) and from the following basic fact on group actions, whose proof is left to the reader.
...
\begin{exampleplain}If $G(\R)$ is the symplectic group $\mathrm{Sp}(2n, \R)$, then $Q(G) \simeq \Z/2\Z$ and there are two distinct Whittaker data for $G(\R)$. If $\Pi$ is the $L$-packet of discrete series with the same infinitesimal character as the trivial representation, then $\Pi_{\mathrm{gen}}$ has two elements, and the correspondence $\pi \mapsto \Wh(\pi)$ in Theorem~\ref{th:main} was made explicit by Mœglin and Renard \cite[Proposition~4.1]{moeglin_renard}, based on work of Wallach~\cite{wallach} and Oda~\cite{oda} for $n=1,2$. As we shall see their result agrees with the description in Section~\ref{sec:alternate_WF} below. \end{exampleplain} 


\begin{remarkplain} 
Not all quasi-split real groups have discrete series representations. But if $G(\R)$ is a quasi-split real group, then the  \emph{fundamental series} representations that are generic are irreducibly induced from generic discrete series of a maximal cuspidal Levi subgroup $M(\R)$ of $G(\R)$. Using \cite[Chapter~14]{abv}, especially Lemma 14.11 there, one may relate Whittaker data for $G(\R)$ with Whittaker data for $M(\R)$, and the generic fundamental series representations in a given $L$-packet for $G(\R)$ (together with the attached Whittaker data) with  generic discrete series representations for maximal cuspidal Levi subgroups. We shall not pursue this here. 
 \end{remarkplain} 
...
\subsubsection{Definition of the two groups, and a canonical isomorphism}\label{sec:first_defs_q}
...
\begin{remarkplain}\label{rk:size_Q} When $G$ is adjoint, it immediately follows from the definitions that $(G_\ad)^\sigma = (G^\sigma)_{\ad}$, therefore $Q_\sigma(G)$ is trivial in that case. 

In general, the group $G(\R)_{\ad}=(G^\sigma)_{\ad}$ contains the identity component of~$\Gad(\R)$, because the above injection $(G^\sigma)_{\ad} \hookrightarrow (\Gad)^\sigma$ is an isomorphism on the Lie algebra level. Therefore  $Q_\sigma(G)$ may be viewed as a quotient of the component group of $\Gad(\R)$, and is an elementary abelian $2$-group. 

When $G(\R)$ is connected, which happens for instance if $G$ is simply connected, the group $G(\R)_{\ad}$ is itself connected, and therefore equal to the identity component of $\Gad(\R)$. In that case $Q_\sigma(\R)$ is the component group of $\Gad(\R)$.

For the concrete computation of this component group in case  $G$ is simple, see the tables in \cite[Section~10]{galois}. In particular  $Q_\sigma(\R)$ has at most four elements in that case.
\end{remarkplain}
...
$$
Q_\theta(G)=(\Gad)^\theta/(G^\theta)_\ad.
$$
...
\end{lemma}
%
%Therefore we write $Q(G)$ for the group $Q_\sigma(G)$ or~$Q_\theta(G)$, depending on the situation.
%
%The proof uses an interpretation of $Q_\sigma(G)$ and $Q_\theta(G)$ in terms of group cohomology, which we spell out in the next section.
%
%As mentioned in Section \ref{sec:discrete_series} the group $Q$ has an interpretation in terms of the Weyl group.
%
%\begin{lemma}
%  The group $Q(G)$ is isomorphic to $W_{\Kad}/W_K$ (cf. Section \ref{sec:discrete_series}).
%  Consequently $Q(G)$ acts simple transitively on the generic members of any generic discrete series L-packet.
%\end{lemma}
%
%\begin{proof}
%This is straightforward.  We work with $Q_\theta$.
%If $g\in (\Gad)^\theta$, choose $x\in (G^\theta)_{\mathrm{ad}}$ so that
%$x(gTg\inv)x\inv=T$, i.e. $xg\in \Norm_G(T)$.
%
%$xg\in \Norm_G(T)$. This induces a well-defined map
%from $W_{\Kad}$ to $Q_\theta(G)$, and it is easy to see the kernel is precisely $W_K$.
%The last  conclusion is from Lemma \ref{l:bijectiongeneric}.
%\end{proof}
%
%\begin{remarkplain}{The group $Q$ is an elementary abelian $2$-group. It is trivial when $G$ is adjoint, and when $G$ is simple  the cardinality of~$Q$ is at most~$4$. See Remark~\ref{rk:size_Q} for a more precise information.} \end{remarkplain}
%
%\begin{exampleplain}If $G(\R)$ is the symplectic group $\mathrm{Sp}(2n, \R)$, then $Q \simeq \Z/2\Z$ and there are two distinct Whittaker data for $G(\R)$. If $\Pi$ is the $L$-packet of discrete series with the same infinitesimal character as the trivial representation, then $\Pi_{\mathrm{gen}}$ has two elements, and the correspondence $\pi \mapsto \Wh(\pi)$ in Theorem~\ref{th:main} was made explicit by Mœglin and Renard \cite[Proposition~4.1]{moeglin_renard}, based on work of Wallach~\cite{wallach} and Oda~\cite{oda} for $n=1,2$. As we shall see their result agrees with the description in Section~\ref{sec:alternate_WF} below. \end{exampleplain} 
%

We shall prove this in Section~\ref{sec:proof_lemma}, using an interpretation of $Q_\sigma(G)$ and $Q_\theta(G)$ in terms of group cohomology which we  spell out in the next section.
...
By definition every element $w$ in $W_{\Kad}$ has a representative $g$ in $(\Gad)^\theta$; the image of~$g$ in $Q_\theta(G)=(\Gad)^\theta/(G^\theta)_\ad$ depends only on~$w$. This induces a group homomorphism 
 $\psi\colon W_{\Kad} \to Q_\theta(G)$.
 
 \begin{lemma}\label{lem:third_Q}
  The morphism  $\psi$ induces an isomorphism $W_{\Kad}/W_K \to Q_\theta(G)$. 
\end{lemma}
\begin{proof} 
Let us identify $W_{\Kad}$ with a subgroup of $W(G, T)$ as in Section~\ref{sec:discrete_series}. Then $\psi(w)$ is trivial if and only  if $w$ has a representative $g \in \mathrm{Norm}_G(T)$ which is $\theta$-invariant, and this means $w \in W_K$. Therefore the kernel of $\psi$ is $W_K$. 

Given $q \in Q_\theta(G)$ and a representative $g \in (\Gad)^{\theta}$, the $\theta$-invariant maximal tori $g^{-1} T_{\ad} g$ and $T_{\ad}$ of~$\Gad$ are conjugate by an element $x \in (G^\theta)_\ad$; then the element $xg$ of $(\Gad)^\theta$ normalizes $T_{\mathrm{ad}}$, and its image $w$ in $W_{\Kad}$ satisfies $\psi(w)=q$. Thus $\psi$ is surjective.
\end{proof}

Because of Lemmas~\ref{lem:iso_q} and~\ref{lem:third_Q}, when describing the actions in Theorem~\ref{th:main},  we shall realize $Q(G)$  as $W_{\Kad}/W_K$, as $Q_\sigma(G)$ or as~$Q_\theta(G)$, depending on the situation.
...
\end{equation}
The connecting map  $ \psi_\tau\colon \Gad^{\tau} \rightarrow H^1(\tau,Z)$ is defined as follows: if $\gamma \in \Gad^{\tau}$ is the coset $gZ$, then $g\tau(g^{-1}) \in Z^{-\tau}$, and $\psi_\tau$ sends~$\gamma$ to the class of $g \tau(g^{-1})$ in $H^1(\tau, Z)$. The kernel of~$\psi_\tau$ is $(G^\tau)_{\ad}$. Since~\eqref{long_ptset} is exact we deduce that $\psi_\tau$ induces a canonical  bijection
...
\subsubsection{Proof of Lemma~\ref{lem:iso_q}}\label{sec:proof_lemma}
...
Let $\gamma_0$ be the image of $z_0^{-1} k^{-1} g$ in $\Gad$. Then $\gamma_0 \in (G^{\sigma})_\ad$, and $\gamma_0$ is equal to the image of $k^{-1} g$ in $\Gad$. The element $\kappa=\gamma \gamma_0^{-1}$ of $\Gad$  is therefore $\sigma$-invariant, and since $\kappa=kZ$, we also have $(\theta\sigma)(\kappa)=\kappa$; we conclude that $\theta(\kappa) = \kappa$.
...
\subsubsection{On representations} \label{sec:action_DS}
...
Similarly the actions of $\Gad^\theta$ on~$\g$ and~$K$ induce an action of $Q_\theta(G)$ on equivalence classes of $(\g,K)$-modules, for which we use similar notation.
...
There is a similar compatibility statement with the third description of $Q(G)$ as $W_{\Kad}/W_K$, and the corresponding action on equivalence classes of discrete series representations. Suppose $q \in Q_{\sigma}(G)$, and let $w$ be the element of $W_{\Kad}/W_K$ that corresponds to $q$ using the isomorphisms of Lemma~\ref{lem:iso_q} and~\ref{lem:third_Q}. Suppose $\pi=\pi_{\lambda, \zeta}$ is the equivalence class of a discrete series representation of~$G(\R)$; inspecting of the definitions in \cite[Section~4]{Contragredient} shows that $w\cdot \pi = \pi^q$. 

As a result, if~$\Pi$ is an $L$-packet of discrete series for $G(\R)$, we have compatible actions on~$\Pi$ of $Q(G)=W_{\Kad}/W_K$, of $Q_\sigma(G)$, and of $Q_{\theta}(G)$. These actions are all simply transitive by Lemma~\ref{l:bijectiongeneric}.


\subsubsection{On Whittaker data and generic representations}\label{sec:action_whitt}

Suppose $\tilde{q} \in \Gad^\sigma$. For every Whittaker pair $(B, \eta)$ as in Section~\ref{sec:whittaker_defs}, the pair $\tilde q\cdot (B, \eta)=\big(\tilde{q} B \tilde{q}^{-1}, \eta \circ (x \mapsto \tilde{q} x \tilde{q}^{-1})\big)$ is again a Whittaker pair, which is conjugate to $(B, \eta)$ if and only if $\tilde{q} \in (G^\sigma)_{\ad}$. This determines an action of $Q_\sigma(G)$ on the set of Whittaker data for~$G(\R)$, which we shall denote by $(q, \mathfrak{w}) \mapsto q \cdot \mathfrak{w}$. This action is simply transitive: see \cite[Lemma 3.1.3]{adams_kaletha} for an elementary argument.

Now let $\pi$ be a generic discrete series representation of~$G(\R)$. Let $q$ be an element of $Q_\sigma(G)$, with a representative $\tilde{q}$ in $\Gad^\sigma$.  For every Whittaker pair $(B, \eta)$ such that $\pi$ has a $(B, \eta)$-Whittaker model, it is immediate from the definitions that the representation
$\pi^{\tilde g}$ is again a discrete series representation, and that it has a $\tilde{q}\cdot (B, \eta)$-Whittaker model. Passing to equivalence classes we see that $\pi^q$ is generic, and satisfies $\Wh(\pi^q)=q \cdot \Wh(\pi)$. Therefore the map $\pi \mapsto \Wh(\pi)$ is $Q_\sigma(G)$-equivariant.

\begin{comment}
\subsubsection{On generic discrete series in a single $L$-packet}\label{sec:action_transitivity}

Suppose $\Pi$ is an L-packet of discrete series for a quasisplit group, so $\pi$ contains generic representations.
Recall from Section \ref{sec:discrete_series} $\Pi$ is the set of discrete series with
fixed infinitesimal character the $W$-orbit of $\lambda\in \Lambda$ and central character $\zeta$.

\begin{lemma}The group $Q$ acts simply transitively on the generic representations in $\Pi$.
\end{lemma}

\begin{proof}
Recall from Section \ref{sec:discrete_series} that $W$ acts
transitively on $\Pi$, so fix a generic representation  $\pi_{\lambda,\zeta}\in \Pi$.
The central character $\zeta$ plays no role so we drop it from the notation.

We define an action of $Q$ on $\Pi$.
The group $(\Gad)^\theta$ is the image, under projection from $G$ to $\Gad=G/Z$,
of the elements $g\in G$ satisfying $g\inv \theta(g)\in Z$, and we suppress the projection
and consider $Q$ as the quotient of this subgroup of $G$.
For such an element $g$, 
after multiplying by an
element of $K$ we can assume $g\in \Norm_{G}(T)$. Let $w$ be the image of $g$ in $W$, and
define $g\cdot \pi_\lambda=\pi_{w\lambda}$. It is immediate that this defines an action of $Q$ on $\Pi$.

Now suppose $\pi=\pi_\lambda$ is generic, i.e. every simple root in
$\Delta^+(w\lambda)$ is non-compact, and $g\in G$ satisfies $g\inv\theta(g)\in Z$.
As before we can assume
$g\in \Norm_G(T)$, with image $w\in W$.  
The condition on $g$ 
means that $\int(g)$ commutes with $\theta$, so
$\int(g)(X_\alpha)=X_{w\alpha}$ is also non-compact. Therefore $\pi_{w\lambda}$ is generic. 
The converse holds as well: if $\pi_{w\lambda}$ is generic then $g\in (\Gad)^\theta$
where $g\in\Norm_G(T)$ maps to $w$.

Therefore $Q$ acts transitively on the generic elements of $\Pi$. Since the stabilizer of any element of $\Pi$ is $W_K$, and this is the image of the denominator $(G^\theta)_{ad}$ of $Q$, 
it follows immediately that this action is simply transitive.
...
\subsubsection{On regular nilpotent $K$-orbits} \label{sec:action_AV}

Next, $Q(G)$, realized as $Q_\theta(G)$, is a group of automorphisms of $(\g,K)$.
...
In this section only we need to  choose a particular  isomorphism \mbox{$\gder\simeq \gder^*$}.
...
Let $\overline{\mathfrak b}(\R)$ be the kernel of $X$ in $\g(\R)$. This is the Lie algebra of~$\overline{B}(\R)$, where
...
\begin{proposition}[\phantom{}{\cite[Proposition 3.3.3]{adams_kaletha}}]\label{JeffTasho_criterion}
  \label{p:Kostant}
Suppose $\pi$ is a generic discrete series representation of~$G(\R)$. Let $\pi$ be a Whittaker datum for~$G(\R)$, and let~$X \in i\g(\R)^*$ be a regular nilpotent element such that $\w = \w_X$. 
...
By Lemma \ref{l:pi_to_av} the associated variety of a discrete representation only depends on  the
the $W_K$-orbit of any Weyl chamber containing a representative of its Harish-Chandra parameter. 
...
Using Lemma~\ref{lem:Kost_and_AC} again, we see that all these orbits $\mathcal{O}'$ satisfy $F \in \AC(\mathcal{O}')$,
...
We may choose $H_\R, F_\R \in \g$ such that $(H_\R,E_\R,F_\R)$ is an $\mathrm{SL}(2)$-triple and  $E_\R$  is
contained in $i\g(\R)$,
...
\begin{exampleplain}Suppose $G(\R) = \mathrm{Sp}(2n, \R)$ and ~$\pi$ is the generic discrete series representation denoted  $\pi_{\mathbf{B}_+, \mathbf{T}}$ in \cite[Section~4.2]{moeglin_renard}. Take up the notation above and use the discussion in Section~\ref{s:AV} to define a pinning $\mathcal{P}$ of~$G$; then the corresponding Borel subgroup is the subgroup $\textbf{B}_{+}$ of \cite{moeglin_renard}. If $X_\mathcal{P}$ is the principal nilpotent element defined in \cite[Section 3.1]{adams_kaletha}, then the Whittaker data $\mathfrak{w}_{\psi(F_\R)}$  coincides with  $\mathfrak{w}_{X_\mathcal{P}}$: see \cite[Lemma 3.1.2]{adams_kaletha}. By Lemma~\ref{lem:explicit_WF} and Corollary~\ref{cor:matumoto} above we deduce that $\Wh(\pi)$ is the Whittaker datum attached to the pinning~$\mathcal{P}$ and the additive character  $\Lambda\colon x \mapsto e^{ix}$ of~$\R$. This is the Whittaker datum denoted by $\Wh_1$ in \cite[loc. cit.]{moeglin_renard}, therefore our results are consistent with \cite[Proposition~4.1]{moeglin_renard}. 

\end{exampleplain} 


\begin{remarkplain} There is a partial converse to this statement if we consider discrete series within a single $L$-packet. Fix an $L$-packet~$\Pi$ of discrete series, and consider a regular semisimple orbit~$\Omega$ in~$\Op \cap i\g(\R)^\ast$; then we can determine the generic representation~$\pi \in \Pi$ with $\WF(\pi)=\Omega$, as follows. Fix~$\xi_\R \in \Omega$, write $\xi_\R=F_\R^\ast$ with $F_\R \in \g$ and let $E_\R, H_\R \in \g$ be such that $(E_\R, H_\R, F_\R)$ is an  $\mathrm{SL}(2)$-triple with $\theta(E_\R)=-F_\R$. Set
$$
H_\theta=E_\R-F_\R.
$$
