...
Recall $G(\R)$ has relative discrete series representations if and only if it has a relatively compact Cartan subgroup, i.e.  a $\theta$-stable Cartan subgroup~$T$ of~$G$  such that~$T(\R) \cap G_d$ is  compact, where $G_d$ is the derived group of~$G$. Throughout the paper, we assume $G(\R)$ does have discrete series representations, we fix  a Cartan subgroup $T \subset G$ satisfying the conditions above, and we let~$\mathfrak{t}$ denote the Lie algebra of~$T$, which is a Cartan subalgebra of~$\g$. 
...
Suppose $\pi\simeq\pi_{\lambda,\zeta}$ is a discrete series representation; then $\pi$ is generic if and only
if $\Delta^+(\lambda)$ is large.
See~\cite[Section~6]{vogan-gelfand-kirillov}. In the terminology of Shelstad~\cite[Section~2.3]{shelstad}, if $B$~is the corresponding Borel subgroup of~$G$ corresponding to $\Delta^+(\lambda)$, then $(B, T)$ is a fundamental Borel pair of Whittaker type.
