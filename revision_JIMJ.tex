...
\item[(ii)]  $\eta$ is a character of the nilpotent radical~$N(\R)$ of~$B(\R)$, which  is nontrivial on each simple root space.
\end{enumerate}
...
Recall that a representation $\pi$ of~$G(\R)$ is said to {have a $(B,\eta)$-Whittaker model} if it has a nonzero distribution vector on which $N(\R)$ acts by $\eta$ (for a more precise statement of the definition and terminology see Section~\ref{sec:invariants}).
...
and the \emph{associated variety} $\AV(\pi)$, which is
...
\end{enumerate}
Many results in this note were known. We gave new and simplified proofs for as many of them as possible; for instance, Section~\ref{sec:Sekiguchi} contains a simple proof that $\WF(\pi)$ and $\AV(\pi)$ are exchanged by the Kostant--Sekiguchi correspondence. Otherwise we give convenient references.
...
Alternatively, this is equivalent to having a non-degenerate bilinear form on~$\g$ which is both $G(\C)$- and Galois-invariant.
...
We say $\pi$ has a \emph{$(B,\eta)$-Whittaker model}
if there exists a nonzero element $v$ in~$\mH^{-\infty}$
such that $\pi(n)(v)=\eta(n)v$ for all $n$ in the nilradical $N(\R)$ of $B(\R)$.
See~\cite{matumoto}. 
This condition depends only on the $G(\R)$-conjugacy class of the pair $(B, \eta)$.
...
If~$\w$ is the  Whittaker datum corresponding to $(B, \eta)$, i.e. the $G(\R)$-conjugacy class of the pair $(B, \eta)$, we say~$\pi$ has a \emph{$\w$-Whittaker model}.  
...
Recall $G(\R)$ has relative discrete series representations if and only if it has a relatively compact Cartan subgroup, i.e.  a $\theta$-stable Cartan subgroup~$T$ of~$G$  such that~$T(\R) \cap G_d$ is  compact, where $G_d$ is the derived group of~$G$. Throughout the paper, we assume $G(\R)$ has discrete series representations, we fix  a Cartan subgroup $T \subset G$ satisfying the conditions above, and we let~$\mathfrak{t}$ denote the Lie algebra of~$T$, which is a Cartan subalgebra of~$\g$. 

Choose a  positive system~$\Delta^+$  for the roots of~$\t$ in~$\g$, and let~$\rho \in \mathfrak{t}^\ast$ be the half-sum of positive roots. Let~$\Lambda \subset \mathfrak{t}^\ast$ be the set of elements~$\lambda \in \mathfrak{t}^\ast$ which are \emph{regular} (i.e. $\lambda$ does not vanish on any coroot) and such that $\lambda-\rho$  is the differential of a character of~$T(\R)$. The set $\Lambda$ does not depend on the choice of $\Delta^+$, and is preserved by the action of the Weyl group~$W=W(\g, \t)$ on $\t^\ast$. 

Each discrete series representation of~$G(\R)$ has an \emph{infinitesimal character} or
\emph{Harish-Chandra parameter}, which is an element of~$\Lambda/W$.
...
series representation~$\pi_{\lambda, \zeta}$ of~$G(\R)$: see \cite[Section~4]{Contragredient}. Every
discrete series representation arises in this way. The \emph{infinitesimal character} of~$\pi_{\lambda, \zeta}$ is defined to be the image of~$\lambda$ in $\mathfrak{t}^\ast/W$, and the \emph{central character} is $\zeta$. See \cite[Section~8]{AV1} and \cite{Contragredient} for more on the parametrization.

There is a simple criterion for genericity in terms of this parametrization. Let us say a set of positive roots $\Delta^+$, or the corresponding Weyl
chamber, is \emph{large} if every simple root is non-compact.
...
See~\cite[Section~6]{vogan-gelfand-kirillov}. In the terminology of Shelstad~\cite[Section~2.3]{shelstad_inner_forms}, if $B$~is the corresponding Borel subgroup of~$G$ corresponding to $\Delta^+(\lambda)$, then $(B, T)$ is a fundamental Borel pair of Whittaker type.
...
Furthermore the stabilizer of any  $\pi\in \Pi$ is the normal subgroup $W_K=\mathrm{Norm}_K(T)/T$ of~$W$. For all this see \cite[Section~4]{Contragredient}.
...
The Weyl groups of $G$ and $\Gad$ are canonically isomorphic, so we can identify $W_{\Kad}$ with a subgroup of $W=W(G,T)$. It is immediate from the definition that for $g\in \Norm_G(T)$,  the image of $g$ in $W$
...
\begin{lemma}
  ...
  if $w\in W_{\Kad}$. The map $w \mapsto \pi_{w\lambda}$ induces a bijection between $Q=W_{\Kad}/W_K$ and the generic elements of the L-packet~$\Pi$ of $\pi$,
  and the action of~$W$ on~$\Pi$ induces a simply transitive action of $Q$ on~$\Pi$.
\end{lemma}

\begin{proof}
 Suppose $g\in \Norm_G(T)$, with image $w\in W$.
...
\begin{remark}
For alternative descriptions of $Q=W_{\Kad}/W_K$ see Section \ref{sec:first_defs_q}.
...
prove, using elementary arguments, that the bijection \mbox{(3) $\leftrightarrow$ (4)} we give 
...
The theorem then follows immediately from observations (i)-(ii) and from the  elementary fact that if a group~$Q$ acts simply transitively on two sets $A$ and~$B$, then any $Q$-equivariant map from $A$ to $B$ must be bijective. 



\begin{example}
If $G(\R)$ is the symplectic group $\mathrm{Sp}(2n, \R)$, then $Q(G) \simeq \Z/2\Z$ and there are two distinct Whittaker data for $G(\R)$. If $\Pi$ is the $L$-packet of discrete series with the same infinitesimal character as the trivial representation, then $\Pi_{\mathrm{gen}}$ has two elements, and the correspondence $\pi \mapsto \Wh(\pi)$ in Theorem~\ref{th:main} was made explicit by Mœglin and Renard \cite[Proposition~4.1]{moeglin_renard}, based on work of Wallach~\cite{wallach} and Oda~\cite{oda} for $n=1,2$. As we shall see (in Section~\ref{sec:alternate_WF}), this result agrees with our description of  $\pi \mapsto \Wh(\pi)$, given in Section~\ref{s:gds}. \end{example} 
...
\subsection{The groups $Q_\sigma(G)$ and $Q_{\theta}(G)$}\label{s:Q}
...
\begin{remark}\label{rk:size_Q} When $G$ is adjoint, it immediately follows from the definitions that $(G_\ad)^\sigma = (G^\sigma)_{\ad}$, therefore $Q_\sigma(G)$ is trivial in that case. 
...
\end{lemma}
\begin{proof} 
Let us identify $W_{\Kad}$ with a subgroup of $W(G, T)$ as in Section~\ref{sec:discrete_series}. Then for $w \in W_{\Kad}$, the element $\psi(w)$ is trivial if and only  if $w$ has a representative $g \in \mathrm{Norm}_G(T)$ which is $\theta$-invariant, and this means $w \in W_K$. Therefore the kernel of $\psi$ is $W_K$. 
...
\noindent \textbf{Proof of the  Proposition.}\quad 
...
This proves the first assertion in the Proposition; the second assertion follows from the first and Lemma~\ref{lem:Kost_and_AC}.\qed
...
We may choose $H_\R, F_\R \in \g$ such that $(H_\R,E_\R,F_\R)$ is an $\mathrm{SL}(2)$-triple and  $E_\R$ is contained in $i\g(\R)$,
...
Let us fix a generic discrete series representation~$\pi$ and let $\lambda$ be a representative for its Harish-Chandra parameter.
As in Section \ref{s:AV} let $\kappa$ be the Killing form and $\psi\colon\gder\to \gder^*$ be the corresponding isomorphism.
...
\begin{example}
Suppose $G(\R) = \mathrm{Sp}(2n, \R)$ and ~$\pi$ is the generic discrete series representation denoted  $\pi_{\mathbf{B}_+, \mathbf{T}}$ in \cite[Section~4.2]{moeglin_renard}. As in Section~\ref{s:AV},  fix a nonzero root vector~$X_\alpha$  for each simple root $\alpha$ of $\mathbf{T}$ in the Borel subgroup $\mathbf{B}_+$. These determine a pinning $\mathcal{P}=(\mathbf{T}, \mathbf{B}_+, \{X_\alpha\})$ of~$G$. Following \cite[Section 3.1, see before Lemma 3.1.2]{adams_kaletha}, we may attach to $\mathcal{P}$ a principal nilpotent element $X_\mathcal{P}$; with this notation it follows from \cite[Lemma 3.1.2]{adams_kaletha} that the Whittaker data $\mathfrak{w}_{\psi(F_\R)}$  and  $\mathfrak{w}_{X_\mathcal{P}}$ are identical. By Lemma~\ref{lem:explicit_WF} and Corollary~\ref{cor:matumoto}, in the terminology of \cite{adams_kaletha}, this means $\Wh(\pi)$ is the Whittaker datum attached to the pinning~$\mathcal{P}$ and the additive character  $\Lambda\colon x \mapsto e^{ix}$ of~$\R$. The latter is the Whittaker datum denoted~$\Wh_1$ in \cite[Section~4.2]{moeglin_renard}, and therefore our results are consistent with \cite[Proposition~4.1]{moeglin_renard}. 
\end{example} 


There is a partial converse to Lemma~\ref{lem:explicit_WF} if we consider discrete series within a single $L$-packet. Fix an $L$-packet~$\Pi$ of discrete series, and consider a regular semisimple orbit~$\Omega$ in~$\Op \cap i\g(\R)^\ast$; then we can determine the generic representation~$\pi \in \Pi$ with $\WF(\pi)=\Omega$, as follows. Fix~$\xi_\R \in \Omega$, write $\xi_\R=F_\R^\ast$ with $F_\R \in \g$ and let $E_\R, H_\R \in \g$ be such that $(E_\R, H_\R, F_\R)$ is an  $\mathrm{SL}(2)$-triple with $\theta(E_\R)=-F_\R$. Set
$$
H_\theta=E_\R-F_\R.
$$
