...
\end{enumerate}
Most results in this note were known. We gave new and simplified proofs for as many of them as possible; for instance, Section~\ref{sec:Sekiguchi} contains a simple proof that $\WF(\pi)$ and $\AV(\pi)$ are exchanged by the Kostant--Sekiguchi correspondence. Otherwise we give convenient references.
...
\begin{lemma}
Suppose a group $Q$ acts simply transitively on two sets $A$ and~$B$; then any~$Q$-equivariant map from~$A$ to~$B$ is bijective. 
\end{lemma}
